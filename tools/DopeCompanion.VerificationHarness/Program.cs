...
public static class HarnessScenarioRunner
{
    ...
    private static async Task ExecuteScenarioAsync(Window window, string repoRoot, string outputRoot, FloatLslTestOutlet outlet)
    {
        ...
        var study = mainViewModel.StudyShells.FirstOrDefault(
                        item => string.Equals(item.Id, "dope-projected-feed-colorama", StringComparison.OrdinalIgnoreCase))
                    ?? throw new InvalidOperationException("Dope study shell was not found in the catalog.");

        await mainViewModel.ActivateStudyModeAsync(study);
        await Task.Delay(TimeSpan.FromSeconds(1.5));
        CaptureWindow(window, Path.Combine(outputRoot, "dope-main-window-initial.png"));
        ...
        if (calibrationOnly)
        {
            ...
            var calibrationCommandResult = await RunCalibrationCommandsPhaseAsync(
                ...
                window,
                outputRoot);
            CaptureWindow(window, Path.Combine(outputRoot, "dope-main-window-live.png"));

            await File.WriteAllTextAsync(
                Path.Combine(outputRoot, "dope-study-mode-report.txt"),
        ...
        var kioskScreenshotPath = await CaptureQuestScreenshotProofAsync(
            ...
            "dope-main-window-kiosk-proof.png");
        ...
        FloatLslTestOutlet outlet)
    {
        ...
        var readyQuestScreenshotPath = await CaptureQuestScreenshotProofAsync(
            ...
            "dope-main-window-validation-ready-proof.png");
        ...
        var endedQuestScreenshotPath = await CaptureQuestScreenshotProofAsync(
            ...
            "dope-main-window-validation-ended-proof.png");
        ...
        string fallbackScreenshotPath)
    {
        try
        {
            ...
            await WaitForConditionAsync(
                ...
                TimeSpan.FromSeconds(20),
                "The Dope runtime stayed foregrounded after Exit Kiosk Runtime.");
            ...
            var homeScreenshotPath = await CaptureQuestScreenshotProofAsync(
                ...
                "dope-main-window-home-proof.png");
        ...
        FloatLslTestOutlet outlet)
    {
        ...
        var readyQuestScreenshotPath = await CaptureQuestScreenshotProofAsync(
            ...
            "dope-main-window-participant-ready-proof.png");
        ...
        var runningQuestScreenshotPath = await CaptureQuestScreenshotProofAsync(
            ...
            "dope-main-window-participant-running-proof.png");
        ...
        catch (TimeoutException)
        {
            ...
            // the main Dope verification harness from completing a clean on-head pass.
        }
        ...
        var endedQuestScreenshotPath = await CaptureQuestScreenshotProofAsync(
            ...
            "dope-main-window-participant-ended-proof.png");
        ...
        VerificationPersistenceResult verificationPersistence)
    {
        ...
        else
        {
            ...
            builder.AppendLine("The current public Dope telemetry confirmed study.lsl.* inlet connectivity on this pass, but it did not echo that routed biofeedback value back over quest_twin_state.");
        ...
        string outputRoot)
    {
        ...
        {
            throw new InvalidOperationException("Controller-breathing workspace is not available in the Dope shell.");
        }
        ...
        CaptureWindow(window, Path.Combine(outputRoot, "dope-main-window-controller-breathing-tab.png"));
        ...
        var appliedQuestScreenshotPath = await CaptureQuestScreenshotProofAsync(
            ...
            "dope-main-window-controller-breathing-applied.png");
        ...
        string outputRoot)
    {
        ...
        await Application.Current.Dispatcher.InvokeAsync(() => studyViewModel.SelectedPhaseTabIndex = 4);
        await Task.Delay(TimeSpan.FromMilliseconds(450));
        CaptureWindow(mainWindow, Path.Combine(outputRoot, "dope-main-window-calibration-workspace.png"));
        ...
        CaptureWindow(experimentWindow, Path.Combine(outputRoot, "dope-experiment-session-calibration.png"));
        ...
        string outputRoot)
    {
        ...
        {
            throw new InvalidOperationException("Automatic-breathing controls are not available in the Dope shell.");
        }
        ...
        var automaticQuestScreenshotPath = await CaptureQuestScreenshotProofAsync(
            ...
            "dope-main-window-automatic-breathing-automatic.png");
        ...
        var pausedQuestScreenshotPath = await CaptureQuestScreenshotProofAsync(
            ...
            "dope-main-window-automatic-breathing-paused.png");
    ...
    {
        // The Dope operator workflow expects normal wear-sensor behavior unless
    ...
    private static async Task EnsureHeadsetWakeReadyAsync(StudyShellViewModel studyViewModel)
    {
        ...
        throw new InvalidOperationException(
            $"Headset is not ready for Dope launch. {studyViewModel.HeadsetAwakeSummary} {studyViewModel.HeadsetAwakeDetail} " +
        ...
        string stopActionDetail)
    {
                ...
                stopActionLevel))
        {
            return new VerificationPersistenceResult(
                ...
                Detail: $"The Dope run completed, but one or more critical pass conditions failed or the headset identity could not be read back cleanly. Kiosk exit detail: {stopActionDetail}",
                Baseline: null);
        }
...
internal sealed class FloatLslTestOutlet : IDisposable
{
    ...
    private static class LslNative
    {
        ...
        private static string[] BuildCandidateLibraryPaths()
        {
            ...
            AddCandidate(candidates, Environment.GetEnvironmentVariable("DOPE_LSL_DLL"));
            ...
            AddCandidate(candidates, Path.Combine(userProfile, "source", "repos", "Dope", "Dope", "Packages", "com.labstreaminglayer.lsl4unity", "Plugins", "LSL", "Windows", "x64", "lsl.dll"));
