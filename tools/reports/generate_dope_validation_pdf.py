...
def build_cover_page(
    ...
    axis.text(0.045, 0.952, "Dope Session Review", color=COLORS["text"], fontsize=24, fontweight="bold", va="top")
    axis.text(
        0.045,
        0.905,
    ...
    axis.text(0.955, 0.952, "Dope Companion", color=COLORS["cyan"], fontsize=12, ha="right", va="top")
...
def build_clock_alignment_page(pdf: PdfPages, clock_samples: list[ClockAlignmentSample], settings: dict) -> None:
    ...
    figure.text(
        0.07,
        0.875,
        "This page uses the dedicated 10-second Dope alignment probe recorded at experiment start.",
        color=COLORS["muted"],
        fontsize=10.2,
    )
    figure.text(
...
def build_report(session_dir: Path, output_pdf: Path, output_csv: Path | None = None) -> None:
    ...
    with PdfPages(str(output_pdf)) as pdf:
        ...
        metadata["Title"] = "Dope Session Review"
        metadata["Author"] = "Dope Companion"
        metadata["Subject"] = "Dope session review report"
        metadata["Keywords"] = "dope, quest, session, lsl, pdf"
