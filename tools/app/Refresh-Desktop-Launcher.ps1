<#
.SYNOPSIS
    Replaces old launchers with one clean DOPE Companion shortcut on Desktop and Start Menu.
#>
[CmdletBinding()]
param(
    [string]$DesktopPath = [Environment]::GetFolderPath('Desktop'),
    [string]$StartMenuPath = [Environment]::GetFolderPath('Programs'),
    [string]$ShortcutName = 'DOPE Companion.lnk',
...
$obsoleteLaunchers = @(
    'Launch DOPE Companion.cmd',
    'DOPE Companion Launcher.lnk',
    'DOPE Companion.lnk'
)
...
function New-LauncherShortcut {
    ...
    $shortcut.Description = 'Launch DOPE Companion, trying the installed packaged app first and otherwise falling back to the verified single-file publish path'
    $shortcut.Save()
