<#
.SYNOPSIS
    Copies the latest local Dope visual profiles into the repo's bundled release profile folder.
#>
[CmdletBinding()]
param(
    [string]$SourceRoot = (Join-Path $env:LOCALAPPDATA 'DopeCompanion\dope-visual-profiles'),
    [string]$DestinationRoot = '',
    [string]$StudyId = 'dope-projected-feed-colorama'
)
...
if (-not (Test-Path $SourceRoot)) {
    throw "Local Dope visual profile root not found: $SourceRoot"
}
...
Write-Host ("Bundled {0} Dope visual profile(s) into {1}" -f $written.Count, $DestinationRoot) -ForegroundColor Green
