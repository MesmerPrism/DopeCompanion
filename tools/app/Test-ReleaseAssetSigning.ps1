<#
.SYNOPSIS
    Validates Authenticode signing on the published setup EXE and MSIX package.
#>
[CmdletBinding()]
param(
...
function Assert-MsixPayloadLayout {
    ...
    try {
        ...
        $requiredEntries = @(
            'DopeCompanion.App/DopeCompanion.exe',
            'DopeCompanion.App/DopeCompanion.dll'
        )
...
function Assert-PackagedExecutablePayloadSigning {
    ...
    $resolvedPath = (Resolve-Path $Path).Path
    $archive = [System.IO.Compression.ZipFile]::OpenRead($resolvedPath)
    $tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("dope-companion-payload-signing-" + [guid]::NewGuid().ToString('N'))
    New-Item -ItemType Directory -Path $tempRoot -Force | Out-Null

    try {
        $payloadEntries = @(
            $archive.Entries |
                Where-Object { $_.FullName -match '^DopeCompanion\.App/.+\.(dll|exe)$' }
        )
...
$reports = @(
    (Get-SigningReport -Path $PreviewSetupPath -AllowSelfSigned:$AllowSelfSignedPreviewSetup |
        Select-Object @{ Name = 'Asset'; Expression = { 'Setup helper' } }, *)
...
foreach ($report in $selfIssuedReports) {
    $message = switch ($report.Asset) {
        'Setup helper' {
            "Setup bootstrapper is signed with a self-issued certificate. " +
        ...
        'Windows package' {
            ...
            "That remains workable only after the signing certificate is trusted explicitly for sideloading."
        }
        default {
