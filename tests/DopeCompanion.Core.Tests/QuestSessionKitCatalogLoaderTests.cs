    ...
    public async Task LoadAsync_MapsRepoStyleCatalogStructure()
    {
      ...
      public async Task LoadAsync_Discovers_Unmapped_Apks_From_Apk_Directory()
      {
        ...
        try
        {
          ...
          var twinApp = Assert.Single(
            catalog.Apps,
            app => string.Equals(app.ApkFile, "DynamicOscillatoryPatternEntrainment-ProjectedFeedColoramaQuad.apk", StringComparison.OrdinalIgnoreCase));

          Assert.Equal("com.tillh.dynamicoscillatorypatternentrainmentprojectedfeedcoloramaquad", twinApp.PackageId);
          Assert.Contains("dope", twinApp.Tags, StringComparer.OrdinalIgnoreCase);
        }
        finally
        {
          Directory.Delete(root, recursive: true);
      ...
      public async Task LoadAsync_Applies_Hash_Based_Compatibility_Metadata()
      {
        ...
        try
        {
          ...
          var twinApp = Assert.Single(
            catalog.Apps,
            app => string.Equals(app.PackageId, "com.tillh.dynamicoscillatorypatternentrainment", StringComparison.OrdinalIgnoreCase));

          Assert.Equal(DopeCompanion.Core.Models.ApkCompatibilityStatus.Compatible, twinApp.CompatibilityStatus);
    ...
    public async Task WriteAsync_PersistsIndentedManifest()
    {
        ...
        try
        {
            var path = await writer.WriteAsync(new(
                ...
                BrowserUrl: "https://www.aliusresearch.org/dope.html",
                RecentLogs: Array.Empty<DopeCompanion.Core.Models.OperatorLogEntry>()));
    ...
    private static string CreateApkMapCatalogWithUnmappedApk()
    {
        ...
        File.WriteAllText(
            ...
            """
            {
              "apks": [
                {
                  "file": "AlreadyMapped.apk",
                  "packageId": "com.example.alreadymapped"
                }
              ]
            }
            """);

        File.WriteAllBytes(Path.Combine(root, "APKs", "AlreadyMapped.apk"), [0x50, 0x4B, 0x03, 0x04]);
        File.WriteAllBytes(Path.Combine(root, "APKs", "DynamicOscillatoryPatternEntrainment-ProjectedFeedColoramaQuad.apk"), [0x50, 0x4B, 0x03, 0x04]);

        File.WriteAllText(
            Path.Combine(root, "HotloadProfiles", "profiles.json"),
            """
            {
              "defaultPackageIds": [ "com.tillh.dynamicoscillatorypatternentrainment" ],
              "profiles": []
            }
            """);
    ...
    private static string CreateCatalogWithCompatibilityManifest()
    {
        ...
        File.WriteAllText(
            ...
            """
            {
              "apks": [
                {
                  "file": "DynamicOscillatoryPatternEntrainment-ProjectedFeedColoramaQuad.apk",
        ...
        var apkPath = Path.Combine(root, "APKs", "DynamicOscillatoryPatternEntrainment-ProjectedFeedColoramaQuad.apk");
