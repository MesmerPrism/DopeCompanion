...
{
    [Fact]
    public void Inspect_reports_success_when_expected_dope_source_is_visible()
    {
        ...
        [
            new LslVisibleStreamInfo(
                "quest_twin_state",
                "quest.twin.state",
                "dope.quest.com-tillh-dynamicoscillatorypatternentrainment.quest-twin-state.quest-twin-state",
                4,
                0f,
                123d)
        ]);

        var inventory = QuestTwinStatePublisherInventoryService.Inspect(discovery, "com.tillh.dynamicoscillatorypatternentrainment");
        ...
        Assert.Contains("expected Dope source_id", inventory.Detail, StringComparison.OrdinalIgnoreCase);
    }
