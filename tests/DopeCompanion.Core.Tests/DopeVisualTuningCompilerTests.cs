...
public sealed class DopeVisualTuningCompilerTests
{
    [Fact]
    public void Parse_accepts_bundled_template_document()
    {
        var templateJson = LoadTemplateJson();
        var compiler = new DopeVisualTuningCompiler(templateJson);

        var document = compiler.Parse(templateJson);

        Assert.Equal(DopeVisualTuningCompiler.ExpectedSchemaVersion, document.SchemaVersion);
        Assert.Equal(DopeVisualTuningCompiler.ExpectedDocumentKind, document.DocumentKind);
        Assert.Equal(DopeVisualTuningCompiler.ExpectedPackageId, document.PackageId);
    ...
    {
        var compiler = new DopeVisualTuningCompiler(LoadTemplateJson());
        ...
        Assert.Equal(DopeVisualTuningCompiler.ExpectedHotloadTargetKey, compiled.HotloadTargetKey);
        Assert.Contains(compiled.Entries, entry => entry.Key == DopeVisualTuningCompiler.ExpectedHotloadTargetKey);
    ...
    public void Evaluate_confirmation_reports_confirmed_waiting_and_mismatch()
    {
        ...
        var applyRecord = new DopeVisualProfileApplyRecord(
            "profile-a",
            "Profile A",
            "HASH",
            "JSONHASH",
    ...
    public void Evaluate_confirmation_waits_for_changed_field_while_report_is_still_pre_apply_value()
    {
        ...
        Assert.Equal(DopeVisualConfirmationState.Waiting, changedRow.State);
    ...
    public void Evaluate_confirmation_reports_mismatch_after_grace_period_when_changed_field_never_updates()
    {
        ...
        var changedRow = Assert.Single(result.Rows, row => row.Id == "particle_size_max");
        Assert.Equal(DopeVisualConfirmationState.Mismatch, changedRow.State);
        Assert.True(result.MismatchCount > 0);
    }
    ...
    private static string LoadTemplateJson()
    {
        var path = Path.GetFullPath(Path.Combine(
            ...
            "oscillator-config",
            "llm-tuning",
            "dope-visual-tuning-v1.template.json"));
