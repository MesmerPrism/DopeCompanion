...
public sealed class DopeVisualProfileStoreTests : IDisposable
{
    private readonly string _tempRoot = Path.Combine(Path.GetTempPath(), "vc-dope-visual-tests", Guid.NewGuid().ToString("N"));
    ...
    public async Task Save_and_load_round_trip_profile_library()
    {
        ...
        var store = new DopeVisualProfileStore(compiler, _tempRoot);
