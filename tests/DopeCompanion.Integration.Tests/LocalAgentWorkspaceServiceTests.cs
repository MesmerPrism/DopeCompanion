...
public sealed class LocalAgentWorkspaceServiceTests : IDisposable
{
    private readonly string _tempRoot = Path.Combine(
        Path.GetTempPath(),
        "DopeCompanion.LocalAgentWorkspaceTests",
    ...
    public void EnsureWorkspace_MirrorsCuratedFilesAndGeneratesPrompt()
    {
        ...
        WriteFile(Path.Combine(questRoot, "APKs", "DynamicOscillatoryPatternEntrainment-ProjectedFeedColoramaQuad.apk"), "apk-bytes");
        ...
        WriteFile(Path.Combine(questRoot, "LlmTuningProfiles", "dope-controller-breathing-tuning-v1.template.json"), "{}");

        WriteFile(Path.Combine(studyRoot, "manifest.json"), "{}");
        WriteFile(Path.Combine(studyRoot, "dope-projected-feed-colorama.json"), "{}");
        WriteFile(Path.Combine(studyRoot, "dope-projected-feed-colorama", "visual-profiles", "baseline.json"), "{}");
        ...
        WriteFile(Path.Combine(oscillatorRoot, "llm-tuning", "dope-visual-tuning-v1.template.json"), "{}");
        ...
        Assert.Contains("DOPE_LSL_DLL", envScript, StringComparison.OrdinalIgnoreCase);
