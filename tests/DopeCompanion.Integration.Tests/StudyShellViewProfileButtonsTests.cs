...
public sealed class StudyShellViewProfileButtonsTests
{
    ...
    {
        var visualTemplatePath = AppAssetLocator.TryResolveDopeVisualTuningTemplatePath()
            ?? throw new FileNotFoundException("Could not resolve the Dope visual tuning template.");
        var controllerTemplatePath = AppAssetLocator.TryResolveDopeControllerBreathingTuningTemplatePath()
            ?? throw new FileNotFoundException("Could not resolve the Dope controller-breathing tuning template.");

        var visualCompiler = new DopeVisualTuningCompiler(File.ReadAllText(visualTemplatePath));
        var controllerCompiler = new DopeControllerBreathingTuningCompiler(File.ReadAllText(controllerTemplatePath));
        var visualStore = new DopeVisualProfileStore(visualCompiler);
        var controllerStore = new DopeControllerBreathingProfileStore(controllerCompiler);
        ...
        DopeVisualProfileRecord? visualRecord = null;
        DopeControllerBreathingProfileRecord? controllerRecord = null;
        try
        {
            ...
            await fixture.InvokeAsync(async () =>
            {
                ...
                using var visualWorkspace = new DopeVisualProfilesWorkspaceViewModel(CreateStudy(studyId), new PreviewQuestControlService());
                using var controllerWorkspace = new DopeControllerBreathingProfilesWorkspaceViewModel(CreateStudy(studyId), new PreviewQuestControlService());
                ...
                var visualStartupState = new DopeVisualProfileStartupStateStore(studyId).Load();
                ...
                await WaitForConditionAsync(
                    () => string.Equals(tracerStartup.Text, "10", StringComparison.Ordinal),
                    TimeSpan.FromSeconds(10));
                visualStartupState = new DopeVisualProfileStartupStateStore(studyId).Load();
                Assert.NotNull(visualStartupState);
                Assert.Equal(0.74d, visualStartupState!.ControlValues!["oblateness_by_radius_max"], 6);
                Assert.True(
                ...
                var controllerStartupState = new DopeControllerBreathingProfileStartupStateStore(studyId).Load();
                ...
                await WaitForConditionAsync(
                    () => string.Equals(controllerStartup.Text, "7", StringComparison.Ordinal),
                    TimeSpan.FromSeconds(10));
                controllerStartupState = new DopeControllerBreathingProfileStartupStateStore(studyId).Load();
                Assert.NotNull(controllerStartupState);
                Assert.Equal(7d, controllerStartupState!.ControlValues!["median_window"], 6);
    ...
    private sealed class StudyShellViewHost
    {
        public StudyShellViewHost(
            DopeVisualProfilesWorkspaceViewModel visualProfiles,
            DopeControllerBreathingProfilesWorkspaceViewModel controllerBreathingProfiles)
        {
            VisualProfiles = visualProfiles;
            ControllerBreathingProfiles = controllerBreathingProfiles;
        }

        public DopeVisualProfilesWorkspaceViewModel VisualProfiles { get; }

        public DopeControllerBreathingProfilesWorkspaceViewModel ControllerBreathingProfiles { get; }
