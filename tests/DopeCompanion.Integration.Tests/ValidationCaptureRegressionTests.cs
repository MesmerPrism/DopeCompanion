...
public sealed class ValidationCaptureRegressionTests
{
    ...
    {
        var tempRoot = Path.Combine(Path.GetTempPath(), $"dope-validation-regression-{Guid.NewGuid():N}");
    ...
    {
        var tempRoot = Path.Combine(Path.GetTempPath(), $"dope-empty-pull-{Guid.NewGuid():N}");
    ...
    {
        var tempRoot = Path.Combine(Path.GetTempPath(), $"dope-stop-pullback-{Guid.NewGuid():N}");
        Directory.CreateDirectory(tempRoot);
        ...
        try
        {
            ...
            var task = (Task<OperationOutcome?>)method.Invoke(
                ...
                [
                    ...
                    true,
                    true,
                    "/sdcard/Android/data/com.tillh.dynamicoscillatorypatternentrainment/files/session-regression",
                    false
                ])!;
    ...
    {
        var tempRoot = Path.Combine(Path.GetTempPath(), $"dope-recenter-regression-{Guid.NewGuid():N}");
        Directory.CreateDirectory(tempRoot);
    ...
    {
        var tempRoot = Path.Combine(Path.GetTempPath(), $"dope-upstream-status-{Guid.NewGuid():N}");
        Directory.CreateDirectory(tempRoot);
    ...
    {
        var tempRoot = Path.Combine(Path.GetTempPath(), $"dope-upstream-source-{Guid.NewGuid():N}");
        Directory.CreateDirectory(tempRoot);
    ...
    {
        var type = typeof(StudyShellViewModel).Assembly.GetType("DopeCompanion.App.ViewModels.AutomaticBreathingRequest")
    ...
    {
        var type = typeof(StudyShellViewModel).Assembly.GetType("DopeCompanion.App.ViewModels.StudyTwinCommandRequest")
