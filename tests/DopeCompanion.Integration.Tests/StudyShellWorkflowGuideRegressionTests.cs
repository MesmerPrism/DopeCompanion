...
public sealed class StudyShellWorkflowGuideRegressionTests
{
    ...
    public void PinnedBuildStatus_HidesVerifiedBaselineStateWhenPinnedApkMatches()
    {
        ...
        SetPrivateField(
            ...
            new InstalledAppStatus(
                ...
                "apk-hash",
                "/data/app/dope.apk",
                "Installed Dope APK matches the pinned hash.",
                "Installed Dope APK hash is current."));
        SetPrivateField(
            viewModel,
            "_headsetStatus",
            new HeadsetAppStatus(
                ...
                CpuLevel: 4,
                GpuLevel: 4,
                ForegroundPackageId: "com.tillh.dynamicoscillatorypatternentrainment",
                ...
                HeadsetWifiSsid: "DopeLab",
                HeadsetWifiIpAddress: "192.168.0.55",
                HostWifiSsid: "DopeLab",
        ...
        Assert.Equal("Dope APK matches the pinned hash.", viewModel.PinnedBuildSummary);
        ...
        Assert.DoesNotContain("recorded Dope build", viewModel.PinnedBuildSummary, StringComparison.OrdinalIgnoreCase);
    }
    ...
    public void SoftwareIdentityGate_StaysAdvisoryWhenBaselineSurfacingIsDisabled()
    {
        ...
        SetPrivateField(viewModel, "_pinnedBuildSummary", "Dope APK matches the pinned hash.");
        SetPrivateField(viewModel, "_pinnedBuildDetail", "Study package: com.tillh.dynamicoscillatorypatternentrainment");
        ...
        Assert.Equal("Dope APK matches the pinned hash.", gate.Summary);
        Assert.Contains("Study package: com.tillh.dynamicoscillatorypatternentrainment", gate.Detail, StringComparison.Ordinal);
        Assert.Contains("intentionally hidden in the operator shell", gate.Detail, StringComparison.OrdinalIgnoreCase);
        Assert.DoesNotContain("no verified Dope software baseline", gate.Detail, StringComparison.OrdinalIgnoreCase);
    }
    ...
    {
        var viewModel = CreateViewModel(CreateStudy());
        SetPrivateField(viewModel, "_headsetStatus", CreateConnectedHeadsetStatus("com.tillh.dynamicoscillatorypatternentrainment"));
        SetPrivateField(
            viewModel,
            ...
            new QuestTwinStatePublisherInventory(
                ...
                "Windows can see quest_twin_state / quest.twin.state from the expected Dope source_id.",
                AnyPublisherVisible: true,
                ExpectedPublisherVisible: true,
                ExpectedSourceId: "dope.quest.com-dope-dopeexperiment.quest-twin-state.quest-twin-state",
                ExpectedSourceIdPrefix: "dope.quest.",
                VisiblePublishers: []));
        ...
        SetPrivateField(viewModel, "_pinnedBuildLevel", OperationOutcomeKind.Success);
        SetPrivateField(viewModel, "_pinnedBuildSummary", "Pinned Dope APK matches the recorded Dope build.");
        SetPrivateField(viewModel, "_deviceProfileLevel", OperationOutcomeKind.Success);
        SetPrivateField(viewModel, "_deviceProfileSummary", "Pinned Quest device profile is active.");
        SetPrivateField(viewModel, "_deviceProfileDetail", "Pinned Quest properties match.");
    ...
    public void WifiMatchChecks_ExposeRouterPathWarning_WhenQuestWifiTransportLooksBlocked()
    {
        ...
        SetPrivateField(
            ...
            new QuestWifiTransportDiagnosticsResult(
                ...
                "Selector 192.168.0.55:5555. Headset Wi-Fi DopeLab (192.168.0.55). Host Wi-Fi DopeLab via Wi-Fi (192.168.0.22). The PC and Quest report the same SSID, but Windows could not open TCP port 5555 on the Quest IP. On the failing router this usually means guest Wi-Fi/client isolation, AP isolation, WLAN peer blocking, or a stale endpoint surviving an ADB restart.",
                Selector: "Selector 192.168.0.55:5555.",
                HeadsetWifi: "Headset Wi-Fi DopeLab (192.168.0.55).",
                HostWifi: "Host Wi-Fi DopeLab via Wi-Fi (192.168.0.22).",
