...
public sealed class CliDopeControllerCommandTests
{
    [Fact]
    public async Task Dope_controller_fields_use_public_template_metadata()
    {
        var output = await InvokeCliAsync("dope", "controller", "fields", "--json");
    ...
    [Fact]
    public async Task Dope_controller_help_mentions_calibration_mode_and_acceptance_examples()
    {
        var createHelp = await InvokeCliAsync("dope", "controller", "create", "--help");
        ...
        var updateHelp = await InvokeCliAsync("dope", "controller", "update", "--help");
