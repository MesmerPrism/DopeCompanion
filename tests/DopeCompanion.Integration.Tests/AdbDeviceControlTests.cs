...
public class AdbDeviceControlTests
{
    ...
    [Fact]
    public async Task LaunchApp_starts_dope()
    {
        if (DeviceSkip.ShouldSkipMutating) return;
        var service = await CreateConnectedService();

        var target = new QuestAppTarget(
            "dope", "DOPE", "com.tillh.dynamicoscillatorypatternentrainment", "", "",
            "", "", []);
    ...
    public async Task RawAdb_push_and_read_runtime_profile()
    {
        ...
        var remotePath = "/sdcard/Android/data/com.tillh.dynamicoscillatorypatternentrainment/files/runtime_hotload/integration-test-profile.json";
