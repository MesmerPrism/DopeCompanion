...
public sealed class DopeProfileWorkspaceInvalidEditTests
{
    [Fact]
    public async Task Visual_workspace_loads_read_only_bundled_release_profiles_before_local_profiles()
    {
        ...
        var store = new DopeVisualProfileStore(compiler);
        var studyId = $"visual-bundled-profiles-{Guid.NewGuid():N}";
        var bundledRoot = Path.Combine(Path.GetTempPath(), $"dope-visual-bundle-{Guid.NewGuid():N}");
        var previousBundleRoot = Environment.GetEnvironmentVariable("DOPE_VISUAL_PROFILE_BUNDLE_ROOT");
        var bundledName = $"ZZZ Bundled Visual {Guid.NewGuid():N}";
        var localName = $"ZZZ Local Visual {Guid.NewGuid():N}";

        DopeVisualProfileRecord? localRecord = null;
        try
        {
            ...
            Environment.SetEnvironmentVariable("DOPE_VISUAL_PROFILE_BUNDLE_ROOT", bundledRoot);
            localRecord = await store.CreateFromTemplateAsync(localName);

            using var workspace = new DopeVisualProfilesWorkspaceViewModel(CreateStudy(studyId), new ConnectedQuestControlService());
            await workspace.InitializeAsync();

            Assert.Equal("Bundled Dope Baseline", workspace.Profiles[0].DisplayLabel);
        ...
        {
            Environment.SetEnvironmentVariable("DOPE_VISUAL_PROFILE_BUNDLE_ROOT", previousBundleRoot);
    ...
    public async Task Controller_workspace_loads_read_only_bundled_release_profiles_before_local_profiles()
    {
        ...
        var bundledRoot = Path.Combine(Path.GetTempPath(), $"dope-controller-bundle-{Guid.NewGuid():N}");
        var previousBundleRoot = Environment.GetEnvironmentVariable("DOPE_CONTROLLER_BREATHING_PROFILE_BUNDLE_ROOT");
        var bundledName = $"ZZZ Bundled Controller {Guid.NewGuid():N}";
        var localName = $"ZZZ Local Controller {Guid.NewGuid():N}";

        DopeControllerBreathingProfileRecord? localRecord = null;
        try
        {
            Directory.CreateDirectory(bundledRoot);
            var bundledDocument = compiler.CreateDocument(
            ...
            Environment.SetEnvironmentVariable("DOPE_CONTROLLER_BREATHING_PROFILE_BUNDLE_ROOT", bundledRoot);
            localRecord = await store.CreateFromTemplateAsync(localName);

            using var workspace = new DopeControllerBreathingProfilesWorkspaceViewModel(CreateStudy(studyId), new ConnectedQuestControlService());
            await workspace.InitializeAsync();
        ...
        {
            Environment.SetEnvironmentVariable("DOPE_CONTROLLER_BREATHING_PROFILE_BUNDLE_ROOT", previousBundleRoot);
            DeleteFileIfPresent(localRecord?.FilePath);
            DeleteDirectoryIfPresent(bundledRoot);
            DeleteStudySessionArtifacts(
                studyId,
                "dope-controller-breathing-startup",
                "dope-controller-breathing-apply");
        }
    }
    ...
    public async Task Visual_workspace_loads_the_pinned_launch_profile_into_the_runtime_draft_on_startup()
    {
        ...
        DopeVisualProfileRecord? record = null;
        try
        {
            ...
            new DopeVisualProfileStartupStateStore(studyId).Save(startupState);
    ...
    public async Task Visual_workspace_invalid_edit_only_marks_the_edited_row()
    {
        ...
        try
        {
            ...
            using var workspace = new DopeVisualProfilesWorkspaceViewModel(
                CreateStudy(studyId),
            ...
            await WaitForConditionAsync(() => new DopeVisualProfileApplyStateStore(studyId).Load() is not null, TimeSpan.FromSeconds(10));
            compiledCsvPath = workspace.LastCompiledCsvPath;

            var applyRecord = new DopeVisualProfileApplyStateStore(studyId).Load()
    ...
    public async Task Controller_workspace_valid_single_edit_only_marks_the_edited_row()
    {
        ...
        DopeControllerBreathingProfileRecord? record = null;
        string? compiledCsvPath = null;
        try
        {
            record = await store.CreateFromTemplateAsync(profileName);

            using var workspace = new DopeControllerBreathingProfilesWorkspaceViewModel(
                CreateStudy(studyId),
                new ConnectedQuestControlService(),
                new ConnectedTwinModeBridge());
            await workspace.InitializeAsync();
            ...
            workspace.ApplySelectedCommand.Execute(null);
            await WaitForConditionAsync(() => new DopeControllerBreathingProfileApplyStateStore(studyId).Load() is not null, TimeSpan.FromSeconds(10));
            compiledCsvPath = workspace.LastCompiledCsvPath;

            var applyRecord = new DopeControllerBreathingProfileApplyStateStore(studyId).Load()
    ...
    private static DopeVisualProfileFieldViewModel FindVisualField(
        DopeVisualProfilesWorkspaceViewModel workspace,
        string fieldId)
        => workspace.Groups
            .SelectMany(group => group.Fields)
            .First(field => string.Equals(field.Id, fieldId, StringComparison.OrdinalIgnoreCase));

    private static DopeVisualComparisonRowViewModel FindVisualRow(
        DopeVisualProfilesWorkspaceViewModel workspace,
        string fieldId)
        => workspace.ComparisonRows
            .First(row => string.Equals(row.Field.Id, fieldId, StringComparison.OrdinalIgnoreCase));

    private static DopeControllerBreathingProfileFieldViewModel FindControllerField(
        DopeControllerBreathingProfilesWorkspaceViewModel workspace,
        string fieldId)
        => workspace.Groups
            .SelectMany(group => group.Fields)
            .First(field => string.Equals(field.Id, fieldId, StringComparison.OrdinalIgnoreCase));

    private static DopeControllerBreathingComparisonRowViewModel FindControllerRow(
        DopeControllerBreathingProfilesWorkspaceViewModel workspace,
        string fieldId)
        => workspace.ComparisonRows
            .First(row => string.Equals(row.Field.Id, fieldId, StringComparison.OrdinalIgnoreCase));
    ...
    private sealed class ConnectedQuestControlService : IQuestControlService
    {
        ...
        public Task<HeadsetAppStatus> QueryHeadsetStatusAsync(
            ...
            => Task.FromResult(new HeadsetAppStatus(
                ...
                CpuLevel: 2,
                GpuLevel: 2,
                ForegroundPackageId: target?.PackageId ?? "com.tillh.dynamicoscillatorypatternentrainment",
