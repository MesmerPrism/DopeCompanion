...
public class EndToEndSessionTests
{
    ...
    private const string DOPEPackage = "com.tillh.dynamicoscillatorypatternentrainment";
    ...
    public async Task Twin_bridge_config_publish_and_delta_detection()
    {
        ...
        var target = new QuestAppTarget("k", "K", DOPEPackage, "", "", "", "", []);
    ...
    {
        var envRoot = Environment.GetEnvironmentVariable("DOPE_QUEST_SESSION_KIT_ROOT");
        if (!string.IsNullOrEmpty(envRoot) && Directory.Exists(envRoot))
            return envRoot;

        var samples = Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.UserProfile),
            "source", "repos", "DopeCompanion", "samples", "quest-session-kit");
