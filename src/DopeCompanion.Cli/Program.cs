...
public static class Program
{
    ...
    {
        var rootCommand = new RootCommand("DopeCompanion — Quest operator station CLI");
        ...
        rootCommand.AddCommand(BuildDopeCommand());
    ...
    private static Command BuildHotloadCommand()
    {
        ...
        pushCommand.Handler = CommandHandler.Create(async (string profile, string? package, string? root, string? device) =>
        {
            ...
            if (File.Exists(profile))
            {
                ...
                packageId = package ?? "com.tillh.dynamicoscillatorypatternentrainment";
            }
            else
            {
                ...
                packageId = package ?? match.PackageIds.FirstOrDefault() ?? "com.tillh.dynamicoscillatorypatternentrainment";
            }
    ...
    private static Command BuildStudyCommand()
    {
        ...
        var studyArg = new Argument<string>("study", description: "Study shell ID (for example: dope-projected-feed-colorama)");
        ...
        var diagnosticsReportCommand = new Command("diagnostics-report", "Generate a shareable Dope LSL/twin diagnostics report as JSON, LaTeX source, and PDF")
        {
            studyArg,
            rootOption,
            reportJsonOption,
            ...
            string? device) =>
        {
            ...
            try
            {
                ...
                if (!noPdf)
                {
                    pdfOutcome = await GenerateDopeDiagnosticsPdfAsync(result.JsonPath, result.PdfPath).ConfigureAwait(false);
                }
                ...
                DiagnosticsCliSupport.PrintDopeDiagnosticsReport(result, pdfOutcome);
            }
            finally
            {
    ...
    private static Command BuildDopeCommand()
    {
        var dopeCommand = new Command("dope", "Dope profile automation commands that mirror the GUI profile tabs");
        var studyOption = new Option<string>("--study", () => DopeCliSupport.DefaultStudyId, "Study shell ID used for local startup/apply state.");
        var rootOption = new Option<string?>("--root", "Study shell catalog root directory path");
        var jsonOption = new Option<bool>("--json", "Emit JSON output.");
        dopeCommand.AddGlobalOption(studyOption);
        dopeCommand.AddGlobalOption(rootOption);
        dopeCommand.AddGlobalOption(jsonOption);
    ...
    {
        var visual = new Command("visual", "Dope visual-profile commands");

        var list = new Command("list", "List bundled and local Dope visual profiles");
        list.Handler = CommandHandler.Create(async (string study, string? root, bool json) =>
        {
            var profiles = await DopeCliSupport.LoadVisualProfilesAsync(study, root);
            if (json)
            {
                DopeCliSupport.WriteJson(profiles.Select(profile => new
                {
            ...
            {
                DopeCliSupport.PrintVisualProfiles(profiles, study);
            }
        });

        var fields = new Command("fields", "List all Dope visual field ids, ranges, and tooltip metadata");
        fields.Handler = CommandHandler.Create((bool json) =>
        {
            var specs = DopeCliSupport.BuildVisualFieldSpecs();
            if (json)
            {
                DopeCliSupport.WriteJson(specs);
            }
            else
            {
                DopeCliSupport.PrintFieldSpecs("Dope Visual Fields:", specs);
            }
        });

        var profileArg = new Argument<string>("profile", "Visual profile id or name.");
        var show = new Command("show", "Show one Dope visual profile, including its field metadata") { profileArg };
        show.Handler = CommandHandler.Create(async (string profile, string study, string? root) =>
        {
            var profiles = await DopeCliSupport.LoadVisualProfilesAsync(study, root);
            var resolved = DopeCliSupport.ResolveVisualProfile(profiles, profile);
            DopeCliSupport.WriteJson(DopeCliSupport.BuildVisualProfileView(resolved, study));
        });
        ...
        var create = new Command("create", "Create a new Dope visual profile from baseline or another profile");
        ...
        {
            var compiler = DopeCliSupport.CreateVisualCompiler();
            var store = DopeCliSupport.CreateVisualStore(compiler);
            var profiles = await DopeCliSupport.LoadVisualProfilesAsync(study, root);
            var source = DopeCliSupport.ResolveVisualProfile(profiles, from);
            var document = DopeCliSupport.BuildVisualDocument(compiler, source.Record.Document, name, notes, set, scale);
            var saved = await DopeCliSupport.SaveVisualAsNewAsync(store, document);

            DopeCliSupport.StartupSyncResult? sync = null;
            if (setStartup)
            {
                DopeCliSupport.SaveVisualStartupState(study, saved);
                var definition = await ResolveStudyShellAsync(study, root);
                sync = await DopeCliSupport.SyncPinnedStartupProfilesAsync(definition, CreateQuestService(device), study, root, forceWhenStudyNotForeground: false);
            }
            ...
            if (json)
            {
                DopeCliSupport.WriteJson(new
                {
                    created = DopeCliSupport.BuildVisualProfileView(new DopeCliSupport.VisualResolvedProfile(saved, false, false), study),
                    startup_sync = sync
                });
            }
            else
        ...
        var update = new Command("update", "Update an existing local Dope visual profile") { profileArg };
        ...
        update.Handler = CommandHandler.Create(async (string profile, string? name, string? notes, string[] set, string[] scale, bool setStartup, string study, string? root, bool json, string? device) =>
        {
            ...
            var store = DopeCliSupport.CreateVisualStore(compiler);
            var profiles = await DopeCliSupport.LoadVisualProfilesAsync(study, root);
            var target = DopeCliSupport.ResolveVisualProfile(profiles, profile);
            var startupBefore = new DopeVisualProfileStartupStateStore(study).Load();
            var document = DopeCliSupport.BuildVisualDocument(compiler, target.Record.Document, name, notes, set, scale);
            var saved = await DopeCliSupport.SaveVisualExistingAsync(store, target, document);
            ...
            DopeCliSupport.RefreshVisualStateAfterSave(study, target.Record.Id, saved, forceSetStartup: setStartup);
            ...
            if (json)
            {
                DopeCliSupport.WriteJson(new
                {
                    updated = DopeCliSupport.BuildVisualProfileView(new DopeCliSupport.VisualResolvedProfile(saved, false, false), study),
                    startup_sync = sync
                });
            }
            else
        ...
        var delete = new Command("delete", "Delete one local Dope visual profile") { profileArg };
        delete.Handler = CommandHandler.Create(async (string profile, string study, string? root, bool json, string? device) =>
        {
            ...
            {
                throw new InvalidOperationException("Only local Dope visual profiles can be deleted.");
            }

            var startupBefore = new DopeVisualProfileStartupStateStore(study).Load();
            var shouldSyncStartup = string.Equals(startupBefore?.ProfileId, target.Record.Id, StringComparison.OrdinalIgnoreCase);
            DopeCliSupport.ClearVisualStateForDeletedProfile(study, target.Record.Id);
            ...
            if (json)
            {
                DopeCliSupport.WriteJson(new { deleted = target.Record.Id, startup_sync = sync });
            }
            else
            {
        ...
        var importArg = new Argument<string>("path", "Path to a Dope visual profile JSON file.");
        var import = new Command("import", "Import a Dope visual profile JSON file") { importArg };
        import.Handler = CommandHandler.Create(async (string path, string study, bool json) =>
        {
            var store = DopeCliSupport.CreateVisualStore(DopeCliSupport.CreateVisualCompiler());
            var imported = await store.ImportAsync(path);
            if (json)
            {
                DopeCliSupport.WriteJson(new { imported = DopeCliSupport.BuildVisualProfileView(new DopeCliSupport.VisualResolvedProfile(imported, false, false), study) });
            }
            else
            {
        ...
        var export = new Command("export", "Export one Dope visual profile as JSON") { profileArg, exportArg };
        export.Handler = CommandHandler.Create(async (string profile, string path, string study, string? root, bool json) =>
        {
            ...
            if (json)
            {
                DopeCliSupport.WriteJson(new { exported = target.Record.Id, path = Path.GetFullPath(path) });
            }
            else
            {
        ...
        var setStartup = new Command("set-startup", "Set the next-launch Dope visual profile") { profileArg };
        setStartup.Handler = CommandHandler.Create(async (string profile, string study, string? root, bool json, string? device) =>
        {
            var profiles = await DopeCliSupport.LoadVisualProfilesAsync(study, root);
            var target = DopeCliSupport.ResolveVisualProfile(profiles, profile);
            DopeCliSupport.SaveVisualStartupState(study, target.IsBundledBaseline ? null : target.Record);
            var definition = await ResolveStudyShellAsync(study, root);
            var sync = await DopeCliSupport.SyncPinnedStartupProfilesAsync(definition, CreateQuestService(device), study, root, forceWhenStudyNotForeground: false);
            if (json)
            {
                DopeCliSupport.WriteJson(new { startup = target.IsBundledBaseline ? "bundled-baseline" : target.Record.Id, sync });
            }
            else
            {
        ...
        var clearStartup = new Command("clear-startup", "Reset the next-launch Dope visual profile to the bundled baseline");
        clearStartup.Handler = CommandHandler.Create(async (string study, string? root, bool json, string? device) =>
        {
            DopeCliSupport.SaveVisualStartupState(study, null);
            var definition = await ResolveStudyShellAsync(study, root);
            var sync = await DopeCliSupport.SyncPinnedStartupProfilesAsync(definition, CreateQuestService(device), study, root, forceWhenStudyNotForeground: false);
            if (json)
            {
                DopeCliSupport.WriteJson(new { startup = "bundled-baseline", sync });
            }
            else
            {
        ...
        var applyLive = new Command("apply-live", "Apply one Dope visual profile to the current running Dope session") { profileArg };
        applyLive.Handler = CommandHandler.Create(async (string profile, string study, string? root, bool json, string? device) =>
        {
            var definition = await ResolveStudyShellAsync(study, root);
            var profiles = await DopeCliSupport.LoadVisualProfilesAsync(study, root);
            var target = DopeCliSupport.ResolveVisualProfile(profiles, profile);
            var result = await DopeCliSupport.ApplyVisualLiveAsync(definition, CreateQuestService(device), TwinModeBridgeFactory.CreateDefault(), study, target.Record);
            if (json)
            {
                DopeCliSupport.WriteJson(new { outcome = result.Outcome, csv_path = result.CsvPath });
            }
            else
            {
    ...
    private static Command BuildDopeControllerCommand(
        Option<string> studyOption,
        Option<string?> rootOption,
        Option<bool> jsonOption,
        Func<string, string, Option<string[]>> createRepeatedAssignmentOption)
    {
        var controller = new Command("controller", "Dope controller-breathing profile commands, including calibration mode, accepted-motion thresholds, and controller vibration.");

        var list = new Command("list", "List bundled and local Dope controller-breathing profiles");
        list.Handler = CommandHandler.Create(async (string study, string? root, bool json) =>
        {
            var profiles = await DopeCliSupport.LoadControllerProfilesAsync(study, root);
            if (json)
            {
                DopeCliSupport.WriteJson(
                    new[]
                    {
                        new { id = "bundled-baseline", name = "Bundled Dope controller-breathing baseline", kind = "bundled-baseline", path = (string?)null }
                    }
            ...
            {
                DopeCliSupport.PrintControllerProfiles(profiles, study);
            }
        });

        var fields = new Command("fields", "List all Dope controller-breathing field ids, including calibration mode, accepted-motion, and vibration controls.");
        fields.Handler = CommandHandler.Create((bool json) =>
        {
            var specs = DopeCliSupport.BuildControllerFieldSpecs();
            if (json)
            {
                DopeCliSupport.WriteJson(specs);
            }
            ...
            {
                DopeCliSupport.PrintFieldSpecs("Dope Controller-Breathing Fields:", specs);
            }
        });

        var profileArg = new Argument<string>("profile", "Controller-breathing profile id or name.");
        var show = new Command("show", "Show one Dope controller-breathing profile, including its field metadata") { profileArg };
        show.Handler = CommandHandler.Create(async (string profile, string study, string? root) =>
        {
            var profiles = await DopeCliSupport.LoadControllerProfilesAsync(study, root);
            var resolved = DopeCliSupport.ResolveControllerProfile(profiles, profile, allowBaselineTemplate: true);
            DopeCliSupport.WriteJson(DopeCliSupport.BuildControllerProfileView(resolved, study));
        });
        ...
        var create = new Command("create", "Create a new Dope controller-breathing profile from baseline or another profile, including calibration mode, accepted-motion thresholds, and vibration.");
        create.AddOption(fromOption);
        create.AddOption(nameOption);
        create.AddOption(notesOption);
        create.AddOption(setOption);
        ...
        create.Handler = CommandHandler.Create(async (string from, string name, string? notes, string[] set, string[] scale, bool setStartup, string study, string? root, bool json, string? device) =>
        {
            var compiler = DopeCliSupport.CreateControllerCompiler();
            var store = DopeCliSupport.CreateControllerStore(compiler);
            var profiles = await DopeCliSupport.LoadControllerProfilesAsync(study, root);
            var source = DopeCliSupport.ResolveControllerProfile(profiles, from, allowBaselineTemplate: true);
            var document = DopeCliSupport.BuildControllerDocument(compiler, source.Record.Document, name, notes, set, scale);
            var saved = await DopeCliSupport.SaveControllerAsNewAsync(store, document);

            DopeCliSupport.StartupSyncResult? sync = null;
            if (setStartup)
            {
                DopeCliSupport.SaveControllerStartupState(study, saved);
                var definition = await ResolveStudyShellAsync(study, root);
                sync = await DopeCliSupport.SyncPinnedStartupProfilesAsync(definition, CreateQuestService(device), study, root, forceWhenStudyNotForeground: false);
            }
            ...
            if (json)
            {
                DopeCliSupport.WriteJson(new
                {
                    created = DopeCliSupport.BuildControllerProfileView(new DopeCliSupport.ControllerResolvedProfile(saved, false), study),
                    startup_sync = sync
                });
            }
            else
        ...
        var update = new Command("update", "Update an existing local Dope controller-breathing profile, including calibration mode, accepted-motion thresholds, and vibration.") { profileArg };
        update.AddOption(nameOption);
        update.AddOption(notesOption);
        update.AddOption(setOption);
        update.AddOption(scaleOption);
        ...
        update.Handler = CommandHandler.Create(async (string profile, string? name, string? notes, string[] set, string[] scale, bool setStartup, string study, string? root, bool json, string? device) =>
        {
            ...
            var store = DopeCliSupport.CreateControllerStore(compiler);
            var profiles = await DopeCliSupport.LoadControllerProfilesAsync(study, root);
            var target = DopeCliSupport.ResolveControllerProfile(profiles, profile, allowBaselineTemplate: false);
            var startupBefore = new DopeControllerBreathingProfileStartupStateStore(study).Load();
            var document = DopeCliSupport.BuildControllerDocument(compiler, target.Record.Document, name, notes, set, scale);
            var saved = await DopeCliSupport.SaveControllerExistingAsync(store, target, document);
            var shouldSyncStartup = setStartup || string.Equals(startupBefore?.ProfileId, target.Record.Id, StringComparison.OrdinalIgnoreCase);
            DopeCliSupport.RefreshControllerStateAfterSave(study, target.Record.Id, saved, forceSetStartup: setStartup);
            ...
            if (json)
            {
                DopeCliSupport.WriteJson(new
                {
                    updated = DopeCliSupport.BuildControllerProfileView(new DopeCliSupport.ControllerResolvedProfile(saved, false), study),
                    startup_sync = sync
                });
            }
            else
        ...
        var delete = new Command("delete", "Delete one local Dope controller-breathing profile") { profileArg };
        delete.Handler = CommandHandler.Create(async (string profile, string study, string? root, bool json, string? device) =>
        {
            var compiler = DopeCliSupport.CreateControllerCompiler();
            var store = DopeCliSupport.CreateControllerStore(compiler);
            ...
            var startupBefore = new DopeControllerBreathingProfileStartupStateStore(study).Load();
            var shouldSyncStartup = string.Equals(startupBefore?.ProfileId, target.Record.Id, StringComparison.OrdinalIgnoreCase);
            DopeCliSupport.ClearControllerStateForDeletedProfile(study, target.Record.Id);
            await store.DeleteAsync(target.Record.FilePath);
        ...
        var importArg = new Argument<string>("path", "Path to a Dope controller-breathing profile JSON file.");
        var import = new Command("import", "Import a Dope controller-breathing profile JSON file") { importArg };
        import.Handler = CommandHandler.Create(async (string path, string study, bool json) =>
        {
            var store = DopeCliSupport.CreateControllerStore(DopeCliSupport.CreateControllerCompiler());
            var imported = await store.ImportAsync(path);
            if (json)
            {
                DopeCliSupport.WriteJson(new { imported = DopeCliSupport.BuildControllerProfileView(new DopeCliSupport.ControllerResolvedProfile(imported, false), study) });
            }
            else
            {
        ...
        var exportArg = new Argument<string>("path", "Destination JSON path.");
        var export = new Command("export", "Export one Dope controller-breathing profile as JSON") { profileArg, exportArg };
        export.Handler = CommandHandler.Create(async (string profile, string path, string study, string? root, bool json) =>
        {
            var store = DopeCliSupport.CreateControllerStore(DopeCliSupport.CreateControllerCompiler());
            var profiles = await DopeCliSupport.LoadControllerProfilesAsync(study, root);
            var target = DopeCliSupport.ResolveControllerProfile(profiles, profile, allowBaselineTemplate: true);
            await store.ExportAsync(target.Record.Document, path);
            if (json)
            {
                DopeCliSupport.WriteJson(new { exported = target.Record.Id, path = Path.GetFullPath(path) });
        ...
        var setStartup = new Command("set-startup", "Set the next-launch Dope controller-breathing profile") { profileArg };
        setStartup.Handler = CommandHandler.Create(async (string profile, string study, string? root, bool json, string? device) =>
        {
            var profiles = await DopeCliSupport.LoadControllerProfilesAsync(study, root);
            var target = DopeCliSupport.ResolveControllerProfile(profiles, profile, allowBaselineTemplate: true);
            DopeCliSupport.SaveControllerStartupState(study, target.IsBaselineTemplate ? null : target.Record);
            var definition = await ResolveStudyShellAsync(study, root);
            var sync = await DopeCliSupport.SyncPinnedStartupProfilesAsync(definition, CreateQuestService(device), study, root, forceWhenStudyNotForeground: false);
            if (json)
            {
                DopeCliSupport.WriteJson(new { startup = target.IsBaselineTemplate ? "bundled-baseline" : target.Record.Id, sync });
            }
            else
            {
        ...
        var clearStartup = new Command("clear-startup", "Reset the next-launch Dope controller-breathing profile to the bundled baseline");
        clearStartup.Handler = CommandHandler.Create(async (string study, string? root, bool json, string? device) =>
        {
            DopeCliSupport.SaveControllerStartupState(study, null);
            var definition = await ResolveStudyShellAsync(study, root);
            var sync = await DopeCliSupport.SyncPinnedStartupProfilesAsync(definition, CreateQuestService(device), study, root, forceWhenStudyNotForeground: false);
            if (json)
            {
        ...
        var applyLive = new Command("apply-live", "Apply one Dope controller-breathing profile to the current running Dope session") { profileArg };
        applyLive.Handler = CommandHandler.Create(async (string profile, string study, string? root, bool json, string? device) =>
        {
            var definition = await ResolveStudyShellAsync(study, root);
            var profiles = await DopeCliSupport.LoadControllerProfilesAsync(study, root);
            var target = DopeCliSupport.ResolveControllerProfile(profiles, profile, allowBaselineTemplate: true);
            var result = await DopeCliSupport.ApplyControllerLiveAsync(definition, CreateQuestService(device), TwinModeBridgeFactory.CreateDefault(), study, target.Record);
            if (json)
            {
                DopeCliSupport.WriteJson(new { outcome = result.Outcome, csv_path = result.CsvPath });
            }
    ...
    private static Command BuildToolingCommand()
    {
        ...
        statusCommand.Handler = CommandHandler.Create(async (bool json, bool checkUpstream) =>
        {
            ...
            if (json)
            {
                Console.WriteLine(System.Text.Json.JsonSerializer.Serialize(status, DopeCliSupport.JsonOptions));
                return;
            }
        ...
        installCommand.Handler = CommandHandler.Create(async (bool json) =>
        {
            ...
            if (json)
            {
                Console.WriteLine(System.Text.Json.JsonSerializer.Serialize(result, DopeCliSupport.JsonOptions));
                return;
            }
    ...
    {
        var windowsEnvCommand = new Command("windows-env", "CLI mirror of Analyze Windows Environment for liblsl, twin transport, bounded diagnostics, and expected upstream stream checks");
        var jsonOption = new Option<bool>("--json", "Write machine-readable JSON output.");
        ...
        var analyzeCommand = new Command("analyze", "Mirror the GUI Analyze Windows Environment check for Windows tooling, bounded liblsl checks, the twin bridge, and expected upstream stream visibility")
        {
            jsonOption,
            streamOption,
            typeOption,
    ...
    {
        var scriptPath = TryResolveDopeDiagnosticsPdfScriptPath();
    ...
    private static string? TryResolveDopeDiagnosticsPdfScriptPath()
    {
        var candidates = new[]
        {
            ...
            Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "tools", "reports", "generate_dope_diagnostics_pdf.py")),
            Path.Combine(Directory.GetCurrentDirectory(), "tools", "reports", "generate_dope_diagnostics_pdf.py"),
            Path.Combine(
    ...
    private static string ResolveCatalogRoot()
    {
        return CliAssetLocator.TryResolveQuestSessionKitRoot()
            ?? throw new InvalidOperationException(
                "No Quest Session Kit catalog root found. Set DOPE_QUEST_SESSION_KIT_ROOT or use --root.");
    }
