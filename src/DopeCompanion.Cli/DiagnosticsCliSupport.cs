...
internal static class DiagnosticsCliSupport
{
    ...
    internal static void PrintDopeDiagnosticsReport(
        ...
        DateTimeOffset checkedAtUtc)
    {
        ...
        if (!pinnedBuildReady)
        {
            detailParts.Add($"Install the pinned Dope APK before debugging LSL; expected {ShortHash(definition.App.Sha256)}, found {ShortHash(installed.InstalledSha256)}.");
        }
        ...
        {
            detailParts.Add("The Dope package is foregrounded per ADB, so this is no longer just a launch-selection problem.");
        }
        else
        {
            detailParts.Add("The pinned Dope package is not currently confirmed in the foreground. Re-check the visible headset scene before blaming LSL.");
        }
