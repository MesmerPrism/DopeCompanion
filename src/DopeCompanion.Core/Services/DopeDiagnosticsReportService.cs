...
public sealed record DopeDiagnosticsReportRequest(
...
public sealed record DopeDiagnosticsReportResult(
    ...
    DopeDiagnosticsReport Report);

public sealed record DopeDiagnosticsReport(
    ...
    DopeQuestSetupSnapshot QuestSetup,
    ...
    IReadOnlyList<DopeDiagnosticsKeyValue> TwinTelemetry,
    IReadOnlyList<DopeDiagnosticsKeyValue> Artifacts,
    OperationOutcomeKind Level,
    string Summary,
    string Detail);

public sealed record DopeQuestSetupSnapshot(
...
public sealed record DopeMachineLslCheckResult(
    string Label,
    OperationOutcomeKind Level,
    string Summary,
    string Detail);

public sealed record DopeMachineLslStateResult(
    OperationOutcomeKind Level,
    string Summary,
    string Detail,
    IReadOnlyList<DopeMachineLslCheckResult> Checks,
    DateTimeOffset CompletedAtUtc);

public sealed record DopeTwinConnectionProbeResult(
    OperationOutcomeKind Level,
    string Summary,
    string Detail,
    bool InletReady,
...
public sealed record DopeCommandAcceptanceResult(
    OperationOutcomeKind Level,
    string Summary,
    string Detail,
    bool Attempted,
...
public sealed record DopeDiagnosticsKeyValue(string Key, string Value);

public sealed class DopeDiagnosticsReportService
{
    ...
    private const string ReportSchemaVersion = "2026-04-15.dope-lsl-twin-diagnostics.v1";
    ...
    public DopeDiagnosticsReportService(
    ...
    public async Task<DopeDiagnosticsReportResult> GenerateAsync(
        DopeDiagnosticsReportRequest request,
        CancellationToken cancellationToken = default)
    {
        ...
        var jsonPath = Path.Combine(reportDirectory, "dope_lsl_twin_diagnostics.json");
        var texPath = Path.Combine(reportDirectory, "dope_lsl_twin_diagnostics.tex");
        var pdfPath = Path.Combine(reportDirectory, "dope_lsl_twin_diagnostics.pdf");
        ...
        var artifacts = new[]
        {
            new DopeDiagnosticsKeyValue("json", jsonPath),
            new DopeDiagnosticsKeyValue("tex", texPath),
            new DopeDiagnosticsKeyValue("pdf", pdfPath)
        };
        ...
        return new DopeDiagnosticsReportResult(
            level,
            summary,
            detail,
            reportDirectory,
    ...
    public static string RenderLatexReport(DopeDiagnosticsReport report)
    {
        ...
        AppendKeyValueTable(builder, "Study Setup", [
            new DopeDiagnosticsKeyValue("study", report.StudyId),
            new DopeDiagnosticsKeyValue("package", report.PackageId),
            new DopeDiagnosticsKeyValue("expected upstream", $"{report.ExpectedLslStreamName} / {report.ExpectedLslStreamType}"),
            new DopeDiagnosticsKeyValue("selector", report.QuestSetup.Selector),
            new DopeDiagnosticsKeyValue("foreground", report.QuestSetup.ForegroundAndSnapshot),
            new DopeDiagnosticsKeyValue("pinned build", report.QuestSetup.PinnedBuild),
            new DopeDiagnosticsKeyValue("device profile", report.QuestSetup.DeviceProfileSummary)
        ]);

        AppendCheckTable(builder, "Windows Environment", report.WindowsEnvironment.Checks.Select(check =>
            new DopeMachineLslCheckResult(check.Label, check.Level, check.Summary, check.Detail)));
        ...
        AppendKeyValueTable(builder, "Quest Wi-Fi Transport", [
            new DopeDiagnosticsKeyValue("level", RenderLevel(report.QuestWifiTransport.Level)),
            new DopeDiagnosticsKeyValue("summary", report.QuestWifiTransport.Summary),
            new DopeDiagnosticsKeyValue("selector", report.QuestWifiTransport.Selector),
            new DopeDiagnosticsKeyValue("headset wifi", report.QuestWifiTransport.HeadsetWifi),
            new DopeDiagnosticsKeyValue("host wifi", report.QuestWifiTransport.HostWifi),
            new DopeDiagnosticsKeyValue("topology", report.QuestWifiTransport.Topology),
            new DopeDiagnosticsKeyValue("ping", report.QuestWifiTransport.Ping),
            new DopeDiagnosticsKeyValue("tcp", report.QuestWifiTransport.Tcp)
        ]);
        AppendKeyValueTable(builder, "Quest Twin Return Path", [
            new DopeDiagnosticsKeyValue("twin-state outlet", QuestTwinStatePublisherInventoryService.RenderForOperator(report.TwinStatePublisherInventory)),
            new DopeDiagnosticsKeyValue("expected inlet", report.TwinConnection.ExpectedInlet),
            new DopeDiagnosticsKeyValue("runtime target", report.TwinConnection.RuntimeTarget),
            new DopeDiagnosticsKeyValue("connected inlet", report.TwinConnection.ConnectedInlet),
            new DopeDiagnosticsKeyValue("counts", report.TwinConnection.Counts),
            new DopeDiagnosticsKeyValue("quest status", report.TwinConnection.QuestStatus),
            new DopeDiagnosticsKeyValue("quest echo", report.TwinConnection.QuestEcho),
            new DopeDiagnosticsKeyValue("return path", report.TwinConnection.ReturnPath),
            new DopeDiagnosticsKeyValue("transport", report.TwinConnection.TransportDetail)
        ]);

        AppendKeyValueTable(builder, "Command Acceptance Probe", [
            new DopeDiagnosticsKeyValue("level", RenderLevel(report.CommandAcceptance.Level)),
            new DopeDiagnosticsKeyValue("summary", report.CommandAcceptance.Summary),
            new DopeDiagnosticsKeyValue("action id", report.CommandAcceptance.ActionId),
            new DopeDiagnosticsKeyValue("sequence", report.CommandAcceptance.Sequence?.ToString(CultureInfo.InvariantCulture) ?? "n/a"),
            new DopeDiagnosticsKeyValue("accepted", report.CommandAcceptance.Accepted ? "yes" : "no"),
            new DopeDiagnosticsKeyValue("detail", report.CommandAcceptance.Detail)
        ]);
    ...
    private async Task<DopeMachineLslStateResult> BuildMachineLslStateResultAsync(
        StudyShellDefinition study,
        CancellationToken cancellationToken)
    {
        ...
        if (runtimeState.Available)
        {
            ...
            clockProbeStreams = await DiscoverSafeAsync(new LslStreamDiscoveryRequest(DopeClockAlignmentStreamContract.ProbeStreamName, DopeClockAlignmentStreamContract.ProbeStreamType), cancellationToken).ConfigureAwait(false);
        }
        ...
        return new DopeMachineLslStateResult(
            level,
            summary,
    ...
    private static DopeQuestSetupSnapshot BuildQuestSetupSnapshot(
        StudyShellDefinition study,
    ...
    private static DopeTwinConnectionProbeResult BuildTwinConnectionProbe(
        StudyShellDefinition study,
        HeadsetAppStatus headset,
        InstalledAppStatus installed,
        DeviceProfileStatus profileStatus,
        ...
        string? deviceSelector)
    {
        ...
        {
            return new DopeTwinConnectionProbeResult(
    ...
    private static DopeTwinConnectionProbeResult BuildTwinConnectionProbeState(
        StudyShellDefinition study,
        HeadsetAppStatus headset,
        InstalledAppStatus installed,
        DeviceProfileStatus profileStatus,
        ...
        DateTimeOffset checkedAtUtc)
    {
        ...
        if (!pinnedBuildReady)
        {
            detailParts.Add($"Install the pinned Dope APK before debugging LSL; expected {ShortHash(study.App.Sha256)}, found {ShortHash(installed.InstalledSha256)}.");
        }

        if (!deviceProfileReady)
        {
            detailParts.Add("Apply the Dope study device profile before participant validation so Wi-Fi ADB, profile guards, and startup assumptions match the release baseline.");
        }
        ...
        {
            detailParts.Add("The headset is publishing twin state back to Windows, but it has not reported an active Dope inlet connection yet.");
        }
        else if (inletReady && !returnPathReady)
        {
            detailParts.Add(headsetForeground && !twinStatePublisher.ExpectedPublisherVisible
                ? "Dope reported an inlet connection, but the Quest twin-state publisher stalled or became undiscoverable on Windows. This is a separate failure mode from a missing inlet or a backgrounded APK."
                : "Dope reported an inlet connection, but the quest_twin_state return path is missing or stale on Windows.");
        }
        else if (!inletReady && !returnPathReady)
        {
            detailParts.Add("Windows-side transport can be healthy while this still fails; this state usually means the headset runtime has not published Dope LSL or twin-state telemetry yet.");
        }
        ...
        detailParts.Add(headsetForeground
            ? "The Dope package is foregrounded per ADB, so this is no longer just a launch-selection problem."
            : "The pinned Dope package is not currently confirmed in the foreground. Re-check the visible headset scene before blaming LSL.");
    ...
    private static async Task<DopeCommandAcceptanceResult> ProbeCommandAcceptanceAsync(
        StudyShellDefinition study,
        LslTwinModeBridge? bridge,
        CancellationToken cancellationToken)
    {
        ...
        {
            return new DopeCommandAcceptanceResult(
                OperationOutcomeKind.Failure,
        ...
        return accepted
            ? new DopeCommandAcceptanceResult(
                OperationOutcomeKind.Success,
    ...
    private static DopeCommandAcceptanceResult BuildSkippedCommandAcceptanceResult(string detail)
        => new(
            OperationOutcomeKind.Preview,
    ...
    private static IReadOnlyList<DopeDiagnosticsKeyValue> BuildTwinTelemetry(LslTwinModeBridge? bridge)
    {
        if (bridge is null)
        {
            return [];
        ...
        return bridge.ReportedSettings
            ...
            .Take(80)
            .Select(entry => new DopeDiagnosticsKeyValue(entry.Key, entry.Value))
            .ToArray();
    }

    private static DopeMachineLslCheckResult BuildMachineLslRuntimeCheck(LslRuntimeState runtimeState)
        => runtimeState.Available
            ? new DopeMachineLslCheckResult(
    ...
    private static DopeMachineLslCheckResult BuildExpectedUpstreamInventoryCheck(
        ...
        string companionTestSourceId)
    {
        ...
        {
            return new DopeMachineLslCheckResult(
    ...
    private DopeMachineLslCheckResult BuildTestSenderServiceCheck(
        StudyShellDefinition study,
        string expectedStreamName,
        string expectedStreamType,
        IReadOnlyList<LslVisibleStreamInfo> companionExpectedStreams)
    {
        ...
        if (_testSignalService.IsRunning)
        {
            ...
            {
                0 => new DopeMachineLslCheckResult(
                    "Companion TEST sender",
                    OperationOutcomeKind.Warning,
    ...
    private static DopeMachineLslCheckResult BuildTwinOutletInventoryCheck(
    ...
    private static DopeMachineLslCheckResult BuildClockAlignmentTransportCheck(
        IReadOnlyList<LslVisibleStreamInfo> clockProbeStreams)
    {
        var stateDetail =
    ...
    private static DopeMachineLslCheckResult BuildPassiveUpstreamMonitorCheck()
        => new(
    ...
    private static string BuildTestSenderSourceId(StudyShellDefinition study)
        => $"dope.companion.study-shell.test.{study.Id}";
    ...
    private static void AppendKeyValueTable(
        ...
        string title,
        IEnumerable<DopeDiagnosticsKeyValue> rows)
    {
    ...
    private static void AppendCheckTable(
        StringBuilder builder,
        string title,
        IEnumerable<DopeMachineLslCheckResult> checks)
    {
        builder.AppendLine(@"\section*{" + EscapeLatex(title) + "}");
