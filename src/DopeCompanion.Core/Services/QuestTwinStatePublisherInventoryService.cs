        ...
        int limit = 8)
    {
        ...
        if (exactMatch is not null)
        {
            return new QuestTwinStatePublisherInventory(
                ...
                $"Windows can see {StreamName} / {StreamType} from the expected Dope source_id {exactMatch.SourceId}.",
        ...
        if (prefixMatches.Length > 0)
        {
            return new QuestTwinStatePublisherInventory(
                OperationOutcomeKind.Warning,
                "Quest twin-state outlet is visible, but not from the expected Dope source_id.",
