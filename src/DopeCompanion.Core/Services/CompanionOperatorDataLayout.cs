...
{
    public const string RootOverrideEnvironmentVariable = "DOPE_OPERATOR_DATA_ROOT";
    private const string CurrentPackagedName = "MesmerPrism.DopeCompanionPreview";
    private const string LegacyPackagedName = "MesmerPrism.DopeCompanion";
    ...
    public static string DopeVisualProfilesRootPath => Path.Combine(RootPath, "dope-visual-profiles");
    public static string DopeControllerBreathingProfilesRootPath => Path.Combine(RootPath, "dope-controller-breathing-profiles");
    ...
    internal static string ResolveRootPath(string localAppDataPath, string? packageFamilyName, string? overrideRoot)
    {
        ...
        {
            return Path.Combine(normalizedLocalAppData, "DopeCompanion");
        }

        var legacyRoot = TryResolveLegacyPackagedRoot(normalizedLocalAppData, packageFamilyName);
        return legacyRoot ?? Path.Combine(normalizedLocalAppData, "Packages", packageFamilyName, "LocalCache", "Local", "DopeCompanion");
    }
    ...
    internal static string? TryResolveLegacyPackagedRoot(string localAppDataPath, string? packageFamilyName, Func<string, bool>? directoryExists = null)
    {
        ...
        var legacyRoot = Path.Combine(localAppDataPath, "Packages", legacyFamilyName, "LocalCache", "Local", "DopeCompanion");
    ...
    private static string NormalizeLocalAppDataPath(string? localAppDataPath)
    {
        ...
        return Path.GetFullPath(Path.Combine(Path.GetTempPath(), "DopeCompanion"));
    }
