...
public sealed class DopeControllerBreathingProfileStore
{
    private readonly DopeControllerBreathingTuningCompiler _compiler;

    public DopeControllerBreathingProfileStore(
        DopeControllerBreathingTuningCompiler compiler,
        string? rootPath = null)
    {
        _compiler = compiler ?? throw new ArgumentNullException(nameof(compiler));
        RootPath = rootPath ?? CompanionOperatorDataLayout.DopeControllerBreathingProfilesRootPath;
    }

    public string RootPath { get; }

    public async Task<IReadOnlyList<DopeControllerBreathingProfileRecord>> LoadAllAsync(CancellationToken cancellationToken = default)
    {
    ...
    public Task<DopeControllerBreathingProfileRecord> CreateFromTemplateAsync(
        string profileName,
        string? notes = null,
    ...
    public async Task<DopeControllerBreathingProfileRecord> SaveAsync(
        string? existingPath,
        string profileName,
        string? notes,
    ...
    public async Task<DopeControllerBreathingProfileRecord> ImportAsync(
        string sourcePath,
        CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrWhiteSpace(sourcePath))
        {
            throw new InvalidDataException("Select a Dope controller-breathing profile JSON file to import.");
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new InvalidDataException("Select a destination path for the exported Dope controller-breathing profile.");
        }
    ...
    private async Task<DopeControllerBreathingProfileRecord> LoadRecordAsync(string path, CancellationToken cancellationToken)
    {
    ...
    {
        var trimmed = string.IsNullOrWhiteSpace(profileName) ? "dope-controller-breathing-profile" : profileName.Trim().ToLowerInvariant();
        ...
        return string.IsNullOrWhiteSpace(slug)
            ? "dope-controller-breathing-profile.json"
            : slug + ".json";
    }
