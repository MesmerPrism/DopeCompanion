...
internal static class LslNativeLibraryResolver
{
    ...
    public static bool TryLoad(out nint libraryHandle, out string detail)
    {
        lock (Sync)
        {
            ...
            detail = $"Could not locate or load lsl.dll. Set DOPE_LSL_DLL or place lsl.dll in the app folder. Searched: {string.Join("; ", CandidateLibraryPaths)}";
            return false;
        }
    }
    ...
    private static string[] BuildCandidateLibraryPaths()
    {
        ...
        AddIfPresent(Environment.GetEnvironmentVariable("DOPE_LSL_DLL"));
        ...
        AddIfPresent(Path.Combine(userProfile, "source", "repos", "Dope", "Dope", "Packages", "com.labstreaminglayer.lsl4unity", "Plugins", "LSL", "Windows", "x64", "lsl.dll"));
