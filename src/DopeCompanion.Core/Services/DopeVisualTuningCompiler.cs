...
public sealed class DopeVisualTuningCompiler
{
    public const string ExpectedSchemaVersion = "dope-visual-tuning/v1";
    public const string ExpectedDocumentKind = "dope_visual_tuning";
    ...
    public DopeVisualTuningCompiler(string templateJson)
    {
        if (string.IsNullOrWhiteSpace(templateJson))
        {
            throw new InvalidDataException("The Dope visual tuning template is empty.");
        }
        ...
        {
            throw new InvalidDataException($"The Dope visual tuning template is not valid JSON: {exception.Message}", exception);
        }

        _templateRoot = rootNode as JsonObject
            ?? throw new InvalidDataException("The Dope visual tuning template must be a JSON object.");
    ...
    public DopeVisualTuningDocument TemplateDocument { get; }

    public DopeVisualTuningDocument CreateDocument(
        string? profileName,
        string? profileNotes,
        IReadOnlyDictionary<string, double>? controlValues = null)
    {
        var controls = new List<DopeVisualTuningControl>(TemplateDocument.Controls.Count);
        ...
        {
            Profile = new DopeVisualTuningProfile(
                string.IsNullOrWhiteSpace(profileName) ? "Dope Visual Profile" : profileName.Trim(),
                string.IsNullOrWhiteSpace(profileNotes) ? null : profileNotes.Trim()),
            Controls = controls
        };
    }

    public DopeVisualTuningDocument Parse(string json)
    {
        if (string.IsNullOrWhiteSpace(json))
        {
            throw new InvalidDataException("The Dope visual tuning file is empty.");
        }
        ...
        {
            throw new InvalidDataException($"The Dope visual tuning file is not valid JSON: {exception.Message}", exception);
        }

        var root = rootNode as JsonObject
            ?? throw new InvalidDataException("The Dope visual tuning file must be a JSON object.");
        root = UpgradeLegacyDocument(root);
        ValidateLockedMetadata(root);
        return ParseDocument(root, validateAgainstTemplate: true);
    }

    public string Serialize(DopeVisualTuningDocument document)
    {
    ...
    public DopeVisualTuningCompileResult Compile(DopeVisualTuningDocument document)
    {
        ArgumentNullException.ThrowIfNull(document);
        ...
        return new DopeVisualTuningCompileResult(
            document,
    ...
    public DopeVisualConfirmationResult EvaluateConfirmation(
        ...
        IReadOnlyDictionary<string, string>? reportedTwinState = null)
    {
        ...
        var rows = new List<DopeVisualConfirmationRow>(TemplateDocument.Controls.Count);
        var confirmed = 0;
        var waiting = 0;
        var mismatch = 0;
        ...
        foreach (var templateControl in TemplateDocument.Controls)
        {
            ...
            {
                state = DopeVisualConfirmationState.Waiting;
                waiting++;
            }
            else if (NearlyEqual(reportedValue.Value, requestedValue))
            {
                state = DopeVisualConfirmationState.Confirmed;
                confirmed++;
            }
            ...
            {
                state = DopeVisualConfirmationState.Mismatch;
                mismatch++;
            }

            rows.Add(new DopeVisualConfirmationRow(
                templateControl.Id,
        ...
        string summary = mismatch > 0
            ? $"Headset reported {confirmed} confirmed, {waiting} waiting, and {mismatch} mismatched Dope parameter values."
            : waiting > 0
                ? $"Waiting on headset confirmation for {waiting} Dope parameter values; {confirmed} already confirmed."
                : $"Headset confirmed all {confirmed} requested Dope parameter values.";

        return new DopeVisualConfirmationResult(summary, rows, confirmed, waiting, mismatch);
    }

    public IReadOnlyList<DopeVisualComparisonRow> BuildComparisonRows(
        DopeVisualTuningDocument selectedDocument,
        DopeVisualTuningDocument? compareDocument = null)
    {
        ...
        var rows = new List<DopeVisualComparisonRow>(TemplateDocument.Controls.Count);

        foreach (var templateControl in TemplateDocument.Controls)
        {
            var selectedControl = selectedControls[templateControl.Id];
            DopeVisualTuningControl? compareControl = null;
    ...
    private DopeVisualTuningDocument ParseDocument(JsonObject root, bool validateAgainstTemplate)
    {
        ...
        if (validateAgainstTemplate)
        {
            ...
            {
                throw new InvalidDataException($"Unsupported Dope visual tuning schema '{schemaVersion}'. Expected '{TemplateDocument.SchemaVersion}'.");
            }

            if (!string.Equals(documentKind, TemplateDocument.DocumentKind, StringComparison.Ordinal))
            {
                throw new InvalidDataException($"Unexpected Dope visual tuning document kind '{documentKind}'. Expected '{TemplateDocument.DocumentKind}'.");
            }
        ...
        var controls = new List<DopeVisualTuningControl>(_controlOrder.Count == 0 ? 8 : _controlOrder.Count);
        ...
        return new DopeVisualTuningDocument(
            schemaVersion,
            documentKind,
            packageId,
            baselineHotloadProfileId,
            hotloadTargetKey,
            new DopeVisualTuningProfile(
                GetRequiredString(profile, "name"),
                GetOptionalString(profile, "notes")),
            controls);
    }

    private DopeVisualTuningControl ParseControl(JsonObject node)
    {
        ...
        return new DopeVisualTuningControl(
            id,
            label,
            editable,
            value,
    ...
    private void ValidateLockedMetadata(JsonObject candidateRoot)
    {
        ...
        if (!JsonNodesEqual(normalizedTemplate, normalizedCandidate))
        {
            throw new InvalidDataException(
                "The Dope visual tuning file changed locked metadata. Only profile.name, profile.notes, and controls.*.value are editable.");
        }
    }
    ...
    private static void ValidateControlPairs(IReadOnlyList<DopeVisualTuningControl> controls)
    {
