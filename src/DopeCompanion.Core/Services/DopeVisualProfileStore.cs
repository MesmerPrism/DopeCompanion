...
public sealed class DopeVisualProfileStore
{
    private readonly DopeVisualTuningCompiler _compiler;

    public DopeVisualProfileStore(
        DopeVisualTuningCompiler compiler,
        string? rootPath = null)
    {
        _compiler = compiler ?? throw new ArgumentNullException(nameof(compiler));
        RootPath = rootPath ?? CompanionOperatorDataLayout.DopeVisualProfilesRootPath;
    }

    public string RootPath { get; }

    public async Task<IReadOnlyList<DopeVisualProfileRecord>> LoadAllAsync(CancellationToken cancellationToken = default)
    {
    ...
    public Task<DopeVisualProfileRecord> CreateFromTemplateAsync(
        string profileName,
        string? notes = null,
    ...
    public async Task<DopeVisualProfileRecord> SaveAsync(
        string? existingPath,
        string profileName,
        string? notes,
    ...
    public async Task<DopeVisualProfileRecord> ImportAsync(
        string sourcePath,
        CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrWhiteSpace(sourcePath))
        {
            throw new InvalidDataException("Select a Dope visual profile JSON file to import.");
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new InvalidDataException("Select a destination path for the exported Dope visual profile.");
        }
    ...
    private async Task<DopeVisualProfileRecord> LoadRecordAsync(string path, CancellationToken cancellationToken)
    {
    ...
    {
        var trimmed = string.IsNullOrWhiteSpace(profileName) ? "dope-visual-profile" : profileName.Trim().ToLowerInvariant();
