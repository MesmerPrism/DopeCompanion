...
{
    private const string LslDiscoveryProbeName = "dope_lsl_discovery_self_check";
    private const string LslDiscoveryProbeType = "dope.diagnostics";
    private const string LslLoopbackProbeNamePrefix = "dope_lsl_loopback_self_check_";
    private const string LslLoopbackProbeType = "dope.loopback";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var checks = new List<WindowsEnvironmentCheckResult>
        {
            ...
            BuildRuntimeCheck(
                ...
                required: true,
                fixHint: "Make sure an official liblsl runtime is available to the app, either from the packaged copy or %DOPE_LSL_DLL%."),
            ...
            BuildRuntimeCheck(
                ...
                required: false,
                fixHint: "The Dope bench sender is optional, but if you use it the same liblsl runtime must be available locally."),
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (completed == delayTask)
        {
            ...
            {
                // Best effort: the caller already has bounded partial diagnostics.
            }
    ...
    private static WindowsEnvironmentCheckResult BuildWindowsPlatformCheck()
    {
        if (OperatingSystem.IsWindows())
        {
            return new WindowsEnvironmentCheckResult(
                ...
                $"Running on {Environment.OSVersion.VersionString}. The public Dope desktop flow expects Windows.");
        }
        ...
        return new WindowsEnvironmentCheckResult(
            ...
            "The Dope desktop operator shell only supports the live public workflow on Windows.");
    }
        ...
        IReadOnlyList<WindowsNetworkAdapterSnapshot> adapters)
    {
        if (!OperatingSystem.IsWindows())
        {
            return new WindowsEnvironmentCheckResult(
                ...
                "The public Dope operator flow is Windows-first; this check is only active on Windows.");
        }
        ...
        if (activeIpv4.Length == 0)
        {
            return new WindowsEnvironmentCheckResult(
                ...
                "LSL discovery and Wi-Fi ADB need a usable Windows network adapter on the same network as the Quest. Connect Wi-Fi or Ethernet before running the Dope checks.");
        }
        ...
        return new WindowsEnvironmentCheckResult(
            ...
            $"{detail}{Environment.NewLine}Hazards: {string.Join("; ", hazards)}. If Dope works intermittently, disable unused VPN, Hyper-V, Docker, WSL, TAP/Wintun, VirtualBox, VMware, or other virtual adapters during the run, keep the Quest and PC on the same Wi-Fi, and use a Private Windows network profile.");
    }
    ...
    private WindowsEnvironmentCheckResult BuildAdbCheck()
    {
        ...
        return new WindowsEnvironmentCheckResult(
            ...
            OperationOutcomeKind.Failure,
            "adb.exe is missing.",
            $"Quest control cannot run until adb.exe is available. Preferred fix: run guided setup or `dope-companion tooling install-official`. Expected managed path: {OfficialQuestToolingLayout.AdbExecutablePath}.");
    }
    ...
    private WindowsEnvironmentCheckResult BuildHzdbCheck()
    {
        ...
        return new WindowsEnvironmentCheckResult(
            ...
            OperationOutcomeKind.Failure,
            "hzdb is missing.",
            $"Quest screenshot, file pullback, and proximity control depend on hzdb. Preferred fix: run guided setup or `dope-companion tooling install-official`. Expected managed path: {OfficialQuestToolingLayout.HzdbExecutablePath}.");
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!_streamDiscoveryService.RuntimeState.Available)
        {
            return new WindowsEnvironmentCheckResult(
                ...
                "Expected Dope LSL stream",
                OperationOutcomeKind.Warning,
                "Expected Dope LSL stream could not be probed.",
