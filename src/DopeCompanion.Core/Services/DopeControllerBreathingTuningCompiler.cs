...
public sealed class DopeControllerBreathingTuningCompiler
{
    public const string ExpectedSchemaVersion = "dope-controller-breathing-tuning/v1";
    public const string ExpectedDocumentKind = "dope_controller_breathing_tuning";
    ...
    public DopeControllerBreathingTuningCompiler(string templateJson)
    {
        if (string.IsNullOrWhiteSpace(templateJson))
        {
            throw new InvalidDataException("The Dope controller-breathing tuning template is empty.");
        }
        ...
        {
            throw new InvalidDataException($"The Dope controller-breathing tuning template is not valid JSON: {exception.Message}", exception);
        }

        _templateRoot = rootNode as JsonObject
            ?? throw new InvalidDataException("The Dope controller-breathing tuning template must be a JSON object.");
    ...
    public DopeControllerBreathingTuningDocument TemplateDocument { get; }

    public DopeControllerBreathingTuningDocument CreateDocument(
        string? profileName,
        string? profileNotes,
        IReadOnlyDictionary<string, double>? controlValues = null)
    {
        var controls = new List<DopeControllerBreathingTuningControl>(TemplateDocument.Controls.Count);
        ...
        {
            Profile = new DopeControllerBreathingTuningProfile(
                string.IsNullOrWhiteSpace(profileName) ? "Dope Controller Breathing Profile" : profileName.Trim(),
                string.IsNullOrWhiteSpace(profileNotes) ? null : profileNotes.Trim()),
            Controls = controls
        };
    }

    public DopeControllerBreathingTuningDocument Parse(string json)
    {
        if (string.IsNullOrWhiteSpace(json))
        {
            throw new InvalidDataException("The Dope controller-breathing tuning file is empty.");
        }
        ...
        {
            throw new InvalidDataException($"The Dope controller-breathing tuning file is not valid JSON: {exception.Message}", exception);
        }

        var root = rootNode as JsonObject
            ?? throw new InvalidDataException("The Dope controller-breathing tuning file must be a JSON object.");
        root = UpgradeLegacyDocument(root);
        ValidateLockedMetadata(root);
        return ParseDocument(root, validateAgainstTemplate: true);
    }

    public string Serialize(DopeControllerBreathingTuningDocument document)
    {
    ...
    public DopeControllerBreathingTuningCompileResult Compile(DopeControllerBreathingTuningDocument document)
    {
        ArgumentNullException.ThrowIfNull(document);
        ...
        return new DopeControllerBreathingTuningCompileResult(document, entries);
    }
    ...
    public DopeControllerBreathingConfirmationResult EvaluateConfirmation(
        DopeControllerBreathingProfileApplyRecord applyRecord,
        IReadOnlyDictionary<string, string> reportedTwinState)
    {
        ...
        var rows = new List<DopeControllerBreathingConfirmationRow>(TemplateDocument.Controls.Count);
        var confirmed = 0;
        var waiting = 0;
        var mismatch = 0;
        ...
        foreach (var templateControl in TemplateDocument.Controls)
        {
            ...
            {
                state = DopeControllerBreathingConfirmationState.Waiting;
                waiting++;
            }
            else if (ValuesMatch(templateControl, reportedValue.Value, requestedValue))
            {
                state = DopeControllerBreathingConfirmationState.Confirmed;
                confirmed++;
            }
            ...
            {
                state = DopeControllerBreathingConfirmationState.Mismatch;
                mismatch++;
            }

            rows.Add(new DopeControllerBreathingConfirmationRow(
                templateControl.Id,
        ...
        return new DopeControllerBreathingConfirmationResult(summary, rows, confirmed, waiting, mismatch);
    }

    public IReadOnlyList<DopeControllerBreathingComparisonRow> BuildComparisonRows(
        DopeControllerBreathingTuningDocument selectedDocument,
        DopeControllerBreathingTuningDocument? compareDocument = null)
    {
        ...
        var rows = new List<DopeControllerBreathingComparisonRow>(TemplateDocument.Controls.Count);

        foreach (var templateControl in TemplateDocument.Controls)
        {
            var selectedControl = selectedControls[templateControl.Id];
            DopeControllerBreathingTuningControl? compareControl = null;
            ...
            rows.Add(new DopeControllerBreathingComparisonRow(
                templateControl.Id,
    ...
    private DopeControllerBreathingTuningDocument ParseDocument(JsonObject root, bool validateAgainstTemplate)
    {
        ...
        if (validateAgainstTemplate)
        {
            ...
            {
                throw new InvalidDataException($"Unsupported Dope controller-breathing tuning schema '{schemaVersion}'. Expected '{TemplateDocument.SchemaVersion}'.");
            }

            if (!string.Equals(documentKind, TemplateDocument.DocumentKind, StringComparison.Ordinal))
            {
                throw new InvalidDataException($"Unexpected Dope controller-breathing tuning document kind '{documentKind}'. Expected '{TemplateDocument.DocumentKind}'.");
            }

            if (!string.Equals(packageId, TemplateDocument.PackageId, StringComparison.Ordinal))
            {
                throw new InvalidDataException($"The tuning file targets '{packageId}', but the Dope compiler only accepts '{TemplateDocument.PackageId}'.");
            }
        }
        ...
        var controls = new List<DopeControllerBreathingTuningControl>(expectedControls.Count);
        ...
        return new DopeControllerBreathingTuningDocument(
            schemaVersion,
            documentKind,
            packageId,
            baselineHotloadProfileId,
            new DopeControllerBreathingTuningProfile(
                GetRequiredString(profile, "name"),
                GetOptionalString(profile, "notes")),
            controls);
    }

    private DopeControllerBreathingTuningControl ParseControl(JsonObject node)
    {
        ...
        return new DopeControllerBreathingTuningControl(
            id,
            group,
            label,
            editable,
            ...
            safeMaximum,
            runtimeKey,
            new DopeControllerBreathingTuningInfo(
    ...
    private void ValidateLockedMetadata(JsonObject candidateRoot)
    {
        ...
        if (!JsonNodesEqual(normalizedTemplate, normalizedCandidate))
        {
            throw new InvalidDataException(
                "The Dope controller-breathing tuning file changed locked metadata. Only profile.name, profile.notes, and controls.*.value are editable.");
        }
    }
    ...
    private double? TryParseReportedValue(
        DopeControllerBreathingTuningControl control,
        IReadOnlyDictionary<string, string> reportedTwinState)
    {
    ...
    private static void ValidateControlPairs(IReadOnlyList<DopeControllerBreathingTuningControl> controls)
    {
