...
{
    public const string AppInstallerDownloadUri = "https://github.com/MesmerPrism/DopeCompanion/releases/latest/download/DopeCompanion.appinstaller";
    public const string ReleasePageUri = "https://github.com/MesmerPrism/DopeCompanion/releases";
    ...
    public PublishedPreviewUpdateService(HttpClient? httpClient = null)
    {
        ...
        {
            _httpClient.DefaultRequestHeaders.UserAgent.ParseAdd("DopeCompanion/1.0");
        }
    }
        ...
        CancellationToken cancellationToken = default)
    {
        var targetDirectory = string.IsNullOrWhiteSpace(destinationDirectory)
            ? Path.Combine(Path.GetTempPath(), "DopeCompanionUpdates")
            : destinationDirectory;
        Directory.CreateDirectory(targetDirectory);

        var targetPath = Path.Combine(targetDirectory, "DopeCompanion.appinstaller");
