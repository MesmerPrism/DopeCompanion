...
public sealed class WindowsAdbQuestControlService : IQuestControlService
{
    ...
    private const string ProfileBrightnessPercentKey = "dope.screen_brightness_percent";
    private const string ProfileMediaVolumeKey = "dope.media_volume_music";
    private const string ProfileHeadsetBatteryMinimumKey = "dope.minimum_headset_battery_percent";
    private const string ProfileRightControllerBatteryMinimumKey = "dope.minimum_right_controller_battery_percent";
    ...
    public async Task<OperationOutcome> LaunchAppAsync(QuestAppTarget target, bool kioskMode = false, CancellationToken cancellationToken = default)
    {
        ...
        if (string.Equals(target.PackageId, DopeExperimentPackage, StringComparison.OrdinalIgnoreCase))
        {
            ...
            if (proximityOutcome.Kind != OperationOutcomeKind.Success)
            {
                return MergeWakeWarning(
                    new OperationOutcome(
                        ...
                        $"The launch path now requires the keep-awake proximity override before starting Dope. {proximityOutcome.Detail}".Trim(),
                        PackageId: target.PackageId),
                    wakeOutcome);
            }
        }
    ...
    internal static OperationOutcome BuildSleepBlockedLaunchOutcome(QuestAppTarget target, QuestWakeReadiness readiness)
        => Failure(
            $"Launch blocked for {target.Label}.",
            $"{readiness.Detail} Wake the headset to enable launching. Do not launch while the headset reports asleep on this HorizonOS build, because Dope can start into a black or limbo scene that may require a headset restart.",
            packageId: target.PackageId);
    ...
    private static string BuildLaunchBlockedWakeInstruction(QuestWakeReadiness readiness)
        => IsSensorLockComponent(readiness.WakeLimboComponent)
            ...
            : "Clear the current Guardian, tracking-loss, or ClearActivity blocker before launching. Do not start Dope while the headset is in this blocked visual state.";
