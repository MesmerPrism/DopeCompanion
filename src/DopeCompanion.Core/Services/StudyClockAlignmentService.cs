...
public sealed class WindowsStudyClockAlignmentService : IStudyClockAlignmentService
{
    ...
    private static readonly TimeSpan WarmPulseInterval = TimeSpan.FromMilliseconds(DopeClockAlignmentStreamContract.DefaultProbeIntervalMilliseconds);
    ...
    public async Task<OperationOutcome> StartWarmSessionAsync(CancellationToken cancellationToken = default)
    {
        ...
        return monitorState switch
        {
            { Connected: true } => new OperationOutcome(
                ...
                $"Reusing {DopeClockAlignmentStreamContract.ProbeStreamName} / {DopeClockAlignmentStreamContract.ProbeStreamType} and keeping the Quest echo path warm with unique keepalive probes every {WarmPulseInterval.TotalMilliseconds:0} ms during the participant session."),
            ...
            _ => new OperationOutcome(
                OperationOutcomeKind.Success,
                "Clock alignment warmup active.",
                $"The probe outlet is active and unique keepalive probes are running every {WarmPulseInterval.TotalMilliseconds:0} ms for {DopeClockAlignmentStreamContract.EchoStreamName} / {DopeClockAlignmentStreamContract.EchoStreamType}.")
        };
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (string.IsNullOrWhiteSpace(request.SessionId) || string.IsNullOrWhiteSpace(request.DatasetHash))
        {
            return new StudyClockAlignmentRunResult(
                new OperationOutcome(
                    ...
                    "Clock alignment requires the active Dope session id and dataset hash before the probe can run."),
                BuildSummary([], probesSent: 0),
                []);
        }
        ...
        progress?.Report(BuildProgress(
            ...
            $"Opening {DopeClockAlignmentStreamContract.ProbeStreamName} / {DopeClockAlignmentStreamContract.ProbeStreamType} and waiting for Quest echoes."));
        ...
        progress?.Report(BuildProgress(
            ...
            monitorConnected
                ? $"Echo inlet for {DopeClockAlignmentStreamContract.EchoStreamName} is ready. Sending probes now."
                : $"Echo inlet did not report ready within {MonitorReadyTimeout.TotalSeconds:0.#} seconds. Sending probes anyway so the Dope flow can continue."));
    ...
    private static OperationOutcome BuildOutcome(StudyClockAlignmentSummary summary)
    {
        if (summary.EchoesReceived <= 0)
        {
            return new OperationOutcome(
                ...
                "The Dope session started, but no round-trip probe completed during the alignment window. Cross-machine clock decomposition is unavailable for this run.");
        }
        ...
        lock (_sync)
        {
            if (_outlet != nint.Zero && _streamInfo != nint.Zero)
            {
                outcome = new OperationOutcome(
                    ...
                    $"Reusing {DopeClockAlignmentStreamContract.ProbeStreamName} / {DopeClockAlignmentStreamContract.ProbeStreamType}.");
                return true;
            }

            _streamInfo = NativeMethods.CreateStreamInfo(
                DopeClockAlignmentStreamContract.ProbeStreamName,
                DopeClockAlignmentStreamContract.ProbeStreamType,
                ProbeChannelCount,
                0d,
                NativeMethods.FloatChannelFormat,
                "dope.companion.dope.clockprobe");
            ...
            outcome = new OperationOutcome(
                OperationOutcomeKind.Success,
                "Clock alignment probe stream active.",
                $"Publishing {DopeClockAlignmentStreamContract.ProbeStreamName} / {DopeClockAlignmentStreamContract.ProbeStreamType}.");
            return true;
        }
    }
    ...
    private sealed class EchoMonitorSession : IDisposable
    {
        ...
        private async Task MonitorAsync(CancellationToken cancellationToken)
        {
            var subscription = new LslMonitorSubscription(
                DopeClockAlignmentStreamContract.EchoStreamName,
                DopeClockAlignmentStreamContract.EchoStreamType,
                0);
    ...
    public Task<OperationOutcome> StartWarmSessionAsync(CancellationToken cancellationToken = default)
        => Task.FromResult(
            new OperationOutcome(
                ...
                "Preview mode does not keep a native Dope clock-alignment transport warm."));
