...
internal sealed record PinnedStartupHotloadSyncResult(
    string? CsvPath,
    DopeVisualStartupHotloadPlan? VisualPlan,
    DopeControllerBreathingStartupHotloadPlan? ControllerPlan,
...
public sealed class StudyShellViewModel : ObservableObject, IDisposable
{
    ...
    private const int WorkflowClockAlignmentDurationSeconds = DopeClockAlignmentStreamContract.DefaultDurationSeconds;
    private const int WorkflowClockAlignmentBackgroundProbeIntervalSeconds = DopeClockAlignmentStreamContract.DefaultBackgroundProbeIntervalSeconds;
    ...
    [
        ...
        new(5, "Verify the Dope APK", "Check whether the approved Dope Experiment APK is installed. If not, install the bundled study APK before moving on."),
        new(6, "Review the device profile", "Apply the pinned CPU, GPU, brightness, and media-volume profile, then review the battery and remaining bench advisories before leaving the bench. These checks stay visible, but they do not block the Dope flow."),
        ...
        new(8, "Launch kiosk mode", "Wake the headset to enable launching, then launch the Dope runtime in kiosk mode. On the current Meta OS build this is best-effort task pinning, not a reliable Meta/menu-button lockout."),
        new(9, "Verify LSL reaches the headset", "Confirm the external heartbeat/biofeedback LSL stream is reaching the Dope runtime and that the resulting live state is visible back in the companion."),
        new(10, "Test particle commands", "Use the companion controls to turn particles on and then off again. This is still recommended for bench confidence, but it does not block the Dope flow."),
        new(11, "Try controller calibration", "Controller-volume breathing calibration is available here, but the current Dope APK path is still unstable. Try it if useful, but it is not required before continuing."),
    ...
    private readonly DopeVisualProfilesWorkspaceViewModel _visualProfiles;
    private readonly DopeControllerBreathingProfilesWorkspaceViewModel _controllerBreathingProfiles;
    ...
    private string _questStatusDetail = "Connect to the headset to verify the Dope study runtime and profile.";
    ...
    private string _pinnedBuildSummary = "Waiting for the bundled Dope APK.";
    private string _pinnedBuildDetail = "The window will compare the bundled Dope APK and the installed Quest build against the Dope study hash.";
    private string _localApkSummary = "Waiting for the bundled Dope APK.";
    private string _localApkDetail = "The Dope shell installs the bundled Dope APK and checks it against the Dope study hash.";
    ...
    private string _liveRuntimeDetail = "Once the study APK starts publishing quest_twin_state, this window will focus on the Dope-specific signals instead of the full raw keyspace.";
    ...
    private string _controllerDetail = "The Dope study expects controller-based breathing. Calibration and controller value reporting will appear here.";
    ...
    private string _testLslSenderDetail = "Start the Windows TEST sender only for bench checks. It publishes smoothed HRV biofeedback samples on an irregular heartbeat-timed profile; Dope treats packet arrival as heartbeat timing and the payload as the routed coherence value.";
    ...
    private string _workflowCurrentStepSummary = "Finish headset setup before starting the Dope protocol.";
    ...
    private string _workflowSetupDetail = "Connect over Wi-Fi ADB, confirm the Dope build, and apply the pinned device profile.";
    ...
    private string _workflowBenchSummary = "Bench verification is waiting for the Dope runtime.";
    ...
    private string _workflowHandoffDetail = "Reset Calibration is part of the Dope shell contract. Finish the handoff, then move into Experiment Session; the actual between-hands sleep and wake path should still end with the physical power button.";
    ...
    private string _workflowRuntimePendingSummary = "Current Dope APK exposes recenter, calibration start, and particle toggles.";
    private string _workflowRuntimePendingDetail =
        @"Scene contract: C:\Users\tillh\source\repos\AstralKarateDojo\Assets\Scenes\DopeControllerStudy.unity. " +
    ...
    private string _clockAlignmentDetail = "Use Start Recording in Experiment Session to capture the dedicated Dope clock-alignment bursts and sparse background drift probes.";
    ...
    public StudyShellViewModel(StudyShellDefinition study)
    {
        ...
        _visualProfiles = new DopeVisualProfilesWorkspaceViewModel(study, _questService, _twinBridge);
        _controllerBreathingProfiles = new DopeControllerBreathingProfilesWorkspaceViewModel(study, _questService, _twinBridge);
    ...
    public string LocalAgentWorkspaceSummary =>
        "Use this folder as the environment for a local agent instead of the protected WindowsApps install. The companion mirrors a bundled CLI, the CLI docs, and the Dope example catalogs there under the host-visible operator-data root.";
    public string LocalAgentWorkspaceDetail =>
        "Open the folder, then paste the built-in prompt into your local agent. The workspace includes `dope-companion.ps1`, `dope-companion.cmd`, and the bundled CLI payload under `cli\\current`, and the wrappers keep the CLI pointed at the same host-visible operator-data root as the installed app.";
    public DopeVisualProfilesWorkspaceViewModel VisualProfiles => _visualProfiles;
    public DopeControllerBreathingProfilesWorkspaceViewModel ControllerBreathingProfiles => _controllerBreathingProfiles;
    ...
    public string StudyApkStepTitle => CanChooseStudyApk ? "2. Verify study APK" : "2. Verify Dope APK";
    public string StudyApkInstallButtonLabel => CanChooseStudyApk ? "Install Study APK" : "Install Dope APK";
    public string StudyApkSourceLabel => CanChooseStudyApk ? "Selected study APK" : "Bundled Dope APK";
        ...
        get
        {
            ...
            return telemetry.AutomaticRoute && telemetry.AutomaticRunning == true
                ...
                : telemetry.AutomaticRoute && telemetry.AutomaticRunning == false
                    ...
                    : telemetry.HasAnyTelemetry
                        ? $"{twinStateDetail} The shell cannot classify this as the Dope controller-volume baseline or the standalone automatic cycle yet.".Trim()
                        : "Live twin-state has not confirmed whether Dope is currently in controller-volume or automatic breathing mode.";
        }
    }
    ...
    private async Task GenerateDiagnosticsReportAsync()
    {
        ...
        try
        {
            ...
            result = await reportService
                .GenerateAsync(new DopeDiagnosticsReportRequest(
                    _study,
    ...
    private async Task<LslMachineStateResult> BuildMachineLslStateResultAsync()
    {
        ...
        if (runtimeState.Available)
        {
            ...
            var clockTask = Task.Run(
                () => _lslStreamDiscoveryService.Discover(new LslStreamDiscoveryRequest(DopeClockAlignmentStreamContract.ProbeStreamName, DopeClockAlignmentStreamContract.ProbeStreamType)),
                CancellationToken.None);
        ...
        IReadOnlyList<LslVisibleStreamInfo> clockProbeStreams)
    {
        var stateDetail =
            ...
            $"Probe stream matches ({DopeClockAlignmentStreamContract.ProbeStreamName} / {DopeClockAlignmentStreamContract.ProbeStreamType}): {clockProbeStreams.Count}{Environment.NewLine}" +
    ...
    private string BuildTestSenderSourceId()
        => $"dope.companion.study-shell.test.{_study.Id}";
    ...
    public async Task InstallStudyAppAsync()
    {
        ...
        if (string.IsNullOrWhiteSpace(localPath) || !File.Exists(localPath))
        {
            await DispatchAsync(() => AppendLog(
                ...
                CanChooseStudyApk
                    ? "Study APK not available locally. Keep the bundled Dope APK with the app or choose the approved Dope APK before installing."
                    : "Bundled Dope APK not available locally. Restore the shipped Dope APK before installing.")).ConfigureAwait(false);
            return;
        }
        ...
        if (!HashMatches(LocalApkHash, _study.App.Sha256))
        {
            await DispatchAsync(() => AppendLog(
                ...
                "Study install blocked.",
                CanChooseStudyApk
                    ? "The selected study APK does not match the Dope study hash. Choose the approved Dope APK before installing."
                    : "The bundled Dope APK does not match the Dope study hash. Replace it with the approved Dope APK before installing.")).ConfigureAwait(false);
            return;
        }

        var outcome = await _questService.InstallAppAsync(CreateStudyTarget(localPath)).ConfigureAwait(false);
        await ApplyOutcomeAsync("Install Dope APK", outcome).ConfigureAwait(false);
    ...
    public async Task LaunchStudyAppAsync()
    {
        ...
        await ApplyOutcomeAsync(
            _study.App.LaunchInKioskMode ? "Launch Dope APK In Kiosk Mode" : "Launch Dope APK",
        ...
        if (outcome.Kind != OperationOutcomeKind.Failure)
        {
            ...
            if (runtimeForeground)
            {
                ...
                if (startupHotloadSync.AppliedToDevice)
                {
                    ...
                    await DispatchAsync(() =>
                    {
                        ...
                        // Dope boot. Runtime draft applies are confirmed separately and do not
            ...
            await DispatchAsync(() =>
            {
                if (!string.Equals(_headsetStatus?.ForegroundPackageId, _study.App.PackageId, StringComparison.OrdinalIgnoreCase))
                {
                    AppendLog(
                        OperatorLogLevel.Warning,
                        "Launch command sent, but Dope is not active.",
    ...
    public async Task StopStudyAppAsync()
    {
        ...
        await ApplyOutcomeAsync(
            _study.App.LaunchInKioskMode ? "Exit Dope Kiosk Mode" : "Stop Dope APK",
            outcome).ConfigureAwait(false);
                ...
                if (runtimeForeground)
                {
                    ...
                    await DispatchAsync(() =>
                        AppendLog(
                            ...
                            "The selected launch profile was saved locally, but the device-side launch file will wait until Dope stops so the current session keeps running unchanged.")).ConfigureAwait(false);
                    return;
                }
        ...
        string reasonLabel)
    {
        ...
        try
        {
            DopeVisualStartupHotloadPlan? visualPlan = null;
            DopeControllerBreathingStartupHotloadPlan? controllerPlan = null;
            await DispatchAsync(() =>
            {
            ...
            if (!string.Equals(reasonLabel, "before launch", StringComparison.OrdinalIgnoreCase))
            {
                await DispatchAsync(() =>
                    AppendLog(
                        ...
                        $"Updated the persistent Dope launch profile file on device from the current pinned startup profiles ({reasonLabel}).")).ConfigureAwait(false);
            }
        ...
        try
        {
            ...
            await ApplyOutcomeAsync(
                ...
                new OperationOutcome(
                    ...
                    $"Open your local agent in {snapshot.RootPath}. This workspace mirrors the bundled CLI, CLI docs, Dope study-shell manifests, device and hotload profiles, tuning templates, and a ready-made agent prompt outside WindowsApps.",
        ...
        try
        {
            ...
            await ApplyOutcomeAsync(
                ...
                new OperationOutcome(
                    ...
                    $"Copied a prompt that tells a local agent to inspect {snapshot.RootPath}, use the bundled CLI wrappers, read the mirrored Dope docs and examples, and explain what the CLI can control today.",
    ...
    private async Task<OperationOutcome?> TryStabilizeStudyPerformancePolicyAsync()
    {
        ...
        var target = CreateStudyTarget(await DispatchAsync(() => StagedApkPath).ConfigureAwait(false));
        var profile = new RuntimeConfigProfile(
            "dope-study-performance-policy",
            "Dope Study Runtime Performance Policy",
            string.Empty,
            DateTime.UtcNow.ToString("yyyy.MM.dd.HHmmss", CultureInfo.InvariantCulture),
            "study",
            false,
            "Disable Dope direct OVR CPU/GPU writes so the pinned ADB device profile remains authoritative.",
            [_study.App.PackageId],
            [
        ...
        if (policyOutcome.Kind == OperationOutcomeKind.Failure)
        {
            return new OperationOutcome(
                OperationOutcomeKind.Warning,
                "Applied study device profile, but Dope may still reset CPU/GPU levels.",
                $"{profileOutcome.Detail} The live runtime performance policy could not be published: {policyOutcome.Detail}");
        }

        return new OperationOutcome(
            profileOutcome.Kind,
            "Applied study device profile and locked Dope runtime CPU/GPU policy.",
            $"{profileOutcome.Detail} Published performance_hint_write_direct_levels=false so the running Dope APK stops overwriting debug.oculus.cpuLevel/gpuLevel.");
    }
        ...
        if (!_controllerBreathingProfiles.IsAvailable)
        {
            await ApplyOutcomeAsync(
                ...
                new OperationOutcome(
                    ...
                    "The Dope controller-breathing profile workspace is not ready yet, so the session window cannot switch between dynamic-axis and fixed-axis calibration on this machine.")).ConfigureAwait(false);
            return;
        }
    ...
    private OperationOutcome? BuildControllerTrackingPreflightFailure(string actionLabel)
    {
        if (_reportedTwinState.Count == 0)
        {
            return new OperationOutcome(
                ...
                "The shell has not received a live quest_twin_state frame yet, so it cannot verify that the active controller is connected and tracked. Probe the connection or wait for a fresh Dope runtime frame, then try calibration again.");
        }
        ...
        if (connected is null || tracked is null)
        {
            return new OperationOutcome(
                ...
                $"The latest Dope state frame does not include selected-controller connected/tracked fields, so the shell cannot verify calibration safety from {frameLabel}. {statusDetail}.{reasonDetail}".Trim());
        }
        ...
        if (connected == false)
        {
            return new OperationOutcome(
                ...
                $"The latest Dope state reports {statusDetail} in {frameLabel}. Connect or wake the selected controller, then try calibration again.{reasonDetail}".Trim());
        }
        ...
        if (tracked == false)
        {
            return new OperationOutcome(
                ...
                $"The latest Dope state reports {statusDetail} in {frameLabel}. Move the selected controller back into tracking view, then try calibration again.{reasonDetail}".Trim());
        }
    ...
    public async Task StartExperimentAsync()
    {
        ...
        recordingSession.RecordEvent(
            ...
            "Participant run requested from the Dope workflow shell.",
    ...
    public async Task EndExperimentAsync()
    {
        ...
        activeSession?.RecordEvent(
            ...
            "Participant wrap-up requested from the Dope workflow shell.",
        ...
        try
        {
            ...
            else
            {
                ...
                var finishOutcome = await FinishParticipantRecordingAsync(
                        ...
                        closedAtUtc,
                        "recording.stopped",
                        "Participant recording closed from the Dope workflow shell before cleanup commands.",
    ...
    private static string? TryResolveValidationCapturePdfScriptPath()
        => new[]
        {
            Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "tools", "reports", "generate_dope_validation_pdf.py")),
            Path.Combine(AppContext.BaseDirectory, "tools", "reports", "generate_dope_validation_pdf.py"),
            Path.Combine(
                ...
                "tools",
                "reports",
                "generate_dope_validation_pdf.py")
        }
    ...
    private async Task<OperationOutcome> GenerateSessionReviewPdfAsync(string localSessionFolderPath, string outputPdfFileName)
    {
        ...
        if (string.IsNullOrWhiteSpace(scriptPath) || !File.Exists(scriptPath))
        {
            return new OperationOutcome(
                ...
                "The companion could not find the bundled Dope session-review PDF generator script.");
        }
            ...
            try
            {
                ...
                if (!ReferenceEquals(completedTask, processExitTask))
                {
                    ...
                    errors.Add(string.IsNullOrWhiteSpace(combinedTimeoutOutput)
                        ? $"{fileName}: Dope session-review PDF generation exceeded {WorkflowValidationPdfTimeout.TotalSeconds:0} seconds and was stopped."
                        : $"{fileName}: Dope session-review PDF generation exceeded {WorkflowValidationPdfTimeout.TotalSeconds:0} seconds and was stopped. {combinedTimeoutOutput}");
                    continue;
                }
        ...
        return new OperationOutcome(
            ...
            errors.Count == 0
                ? "The companion could not launch the Dope session-review PDF generator."
                : string.Join(" ", errors));
    }
    ...
    private async Task<OperationOutcome> PullQuestRecordingArtifactsAsync(string remoteSessionDir, string localSessionFolderPath)
    {
        ...
        using (var listCts = new CancellationTokenSource(WorkflowHzdbListTimeout))
        {
            ...
            catch (OperationCanceledException)
            {
                ...
                diagnostics.Add($"{listCancelReason}; falling back to the known Dope backup file names.");
        ...
        try
        {
            ...
            await DispatchAsync(() =>
            {
                ...
                LocalApkSummary = matches
                    ? CanChooseStudyApk
                        ? "Selected study APK matches the Dope study hash."
                        : "Bundled Dope APK matches the Dope study hash."
                    : CanChooseStudyApk
                        ? "Selected study APK does not match the Dope study hash."
                        : "Bundled Dope APK does not match the Dope study hash.";
        ...
        catch (Exception ex)
        {
            await DispatchAsync(() =>
            {
                ...
                LocalApkSummary = CanChooseStudyApk
                    ...
                    : "Could not verify the bundled Dope APK.";
    ...
    private async Task RefreshHeadsetStatusAsync(bool includeHostWifiStatus = true)
    {
        ...
        await DispatchAsync(() =>
        {
            ...
            else
            {
                ...
                QuestStatusDetail = $"Connected to the headset, but the active app is {foregroundPackage}. Refresh the active app or relaunch the Dope APK before relying on session controls.";
            }
            ...
            _headsetStatus is { IsConnected: true, IsTargetForeground: false })
        {
            ...
            _ = EnsurePinnedStartupHotloadStateAsync(
                target,
                forceWhenStudyNotForeground: true,
                reasonLabel: "after Dope stopped");
        }
    }
    ...
    private void UpdatePinnedBuildStatus()
    {
        ...
        if (surfaceVerifiedBaseline)
        {
            ...
            if (!baselineMatchesPinnedHash)
            {
                ...
                PinnedBuildSummary = "Pinned Dope APK changed after the latest verified run.";
            }
            else if (_installedAppStatus?.IsInstalled == true && hasInstalledHash && !installedMatchesPinnedHash)
            {
                PinnedBuildLevel = OperationOutcomeKind.Failure;
                PinnedBuildSummary = "Headset APK does not match the latest verified Dope build.";
            }
            else if (hasReadyPinnedApkOnHeadset)
            {
                ...
                PinnedBuildSummary = fingerprintMatches || runtimeEnvironmentMatchesBaseline
                    ? "Headset matches the latest verified Dope build."
                    : "Pinned Dope APK matches the recorded Dope build.";
            }
            else
            {
                PinnedBuildLevel = OperationOutcomeKind.Warning;
                PinnedBuildSummary = "Latest verified Dope build recorded. Refresh the headset snapshot to compare it fully.";
            }
        }
        else if (hasInstalledEvidence)
        {
            ...
            {
                PinnedBuildLevel = OperationOutcomeKind.Success;
                PinnedBuildSummary = "Dope APK matches the pinned hash.";
            }
            else if (!hasInstalledHash)
            {
                PinnedBuildLevel = OperationOutcomeKind.Warning;
                PinnedBuildSummary = "Dope runtime is installed, but the headset APK hash could not be verified.";
            }
            else
            {
                PinnedBuildLevel = OperationOutcomeKind.Failure;
                PinnedBuildSummary = "A different Dope APK build is installed on the headset.";
            }
        }
        else if (HashMatches(LocalApkHash, _study.App.Sha256))
        {
            ...
            PinnedBuildSummary = CanChooseStudyApk
                ? "Approved Dope APK is ready to install."
                : "Bundled Dope APK is ready to install.";
        }
        ...
        else
        {
            ...
            PinnedBuildSummary = "Waiting for the bundled Dope APK.";
        }
        ...
        if (surfaceVerifiedBaseline && hasReadyPinnedApkOnHeadset)
        {
            ...
            {
                details.Add("Current headset OS/build differs from the verified baseline. Dope currently keeps software identity as advisory-only bench context.");
            }

            if (!displayMatchesBaseline)
            {
                details.Add("Current headset display id differs from the verified baseline. Dope currently keeps display identity as advisory-only bench context.");
            }
        }
    ...
    private void UpdateHeadsetAwakeStatus(string selector, TrackedQuestProximityState tracked, QuestProximityStatus? liveStatus)
    {
        ...
        else
        {
            ...
            if (IsHeadsetWakeBlockedByLockScreen())
            {
                ...
                HeadsetAwakeDetail = $"{powerEvidence} Quest woke into SensorLockActivity instead of a usable scene. Clear the lock screen on the headset, or press the physical power button again, before you trust the wake state or launch Dope. {proximityContext}".Trim();
            }
            ...
            else if (_headsetStatus.IsAwake == false)
            {
                ...
                HeadsetAwakeDetail = $"{powerEvidence} {proximityContext} {LaunchSleepBlockInstruction} Do not start Dope while the headset is asleep on this Meta OS build, because the runtime can enter a black or limbo scene that may require a headset restart. Other headset action buttons can still wake the device before their command is sent.".Trim();
            }
            else
            {
                HeadsetAwakeLevel = OperationOutcomeKind.Preview;
    ...
    private async Task<StudyDataRecordingSession?> TryBeginParticipantRecordingAsync()
    {
        ...
        try
        {
            ...
            recordingSession.RecordEvent(
                "recording.started",
                "Local Dope participant recording session created.",
                null,
                "success",
                startedAtUtc);
    ...
    private string BuildSessionConditionsJson(DateTimeOffset capturedAtUtc)
    {
        ...
        var snapshot = new
        {
            SchemaVersion = "dope-session-conditions-v1",
    ...
    private async Task<OperationOutcome> PublishParticipantSessionMetadataAsync(StudyDataRecordingSession recordingSession)
    {
        ...
        if (string.IsNullOrWhiteSpace(runtimeConfigJson))
        {
            return new OperationOutcome(
                ...
                "The live Dope runtime has not yet reported showcase_active_runtime_config_json on quest_twin_state, so the participant session metadata cannot be merged into the active runtime config.");
        }
        ...
        var profile = new RuntimeConfigProfile(
            $"dope-study-session-{recordingSession.SessionId}",
            "Dope Study Session Metadata",
            string.Empty,
            DateTime.UtcNow.ToString("yyyy.MM.dd.HHmmss", CultureInfo.InvariantCulture),
            "study",
            false,
    ...
    private OperationOutcome TryStartUpstreamLslMonitor(StudyDataRecordingSession recordingSession)
    {
        ...
        catch (Exception exception)
        {
            return new OperationOutcome(
                ...
                $"The Dope run can continue, but the passive Windows-side LSL monitor could not be armed: {exception.Message}");
        }
    }
            ...
            if (task is not null)
            {
                ...
                if (!ReferenceEquals(completedTask, task))
                {
                    return new OperationOutcome(
                        ...
                        $"The passive Windows-side HRV monitor did not stop within {WorkflowUpstreamMonitorStopTimeout.TotalSeconds:0} seconds. The Dope workflow can continue, but the monitor may still be unwinding.");
                }
            ...
            if (task is not null)
            {
                ...
                if (!ReferenceEquals(completedTask, task))
                {
                    return new OperationOutcome(
                        ...
                        $"The sparse background probe did not stop within {WorkflowClockAlignmentStopTimeout.TotalSeconds:0} seconds. The Dope run can continue, but the background probe may still be unwinding.");
                }
    ...
    private async Task CaptureRegularRecordingSampleAsync(DateTimeOffset recordedAtUtc, CancellationToken cancellationToken)
    {
        ...
        {
            await DispatchAsync(() => SetRecorderFault("Write live Dope telemetry", exception)).ConfigureAwait(false);
        }
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var probeInterval = TimeSpan.FromMilliseconds(DopeClockAlignmentStreamContract.DefaultProbeIntervalMilliseconds);
        ...
        var request = new StudyClockAlignmentRunRequest(
            ...
            duration,
            probeInterval,
            TimeSpan.FromMilliseconds(DopeClockAlignmentStreamContract.DefaultEchoGraceMilliseconds),
            firstProbeSequence);
    ...
    private void ResetClockAlignmentStateForRun(StudyClockAlignmentWindowKind windowKind, TimeSpan duration)
    {
        ...
        {
            StudyClockAlignmentWindowKind.StartBurst => "The companion is sending dedicated Dope clock probes and waiting for the Quest echo stream so the clocks can be aligned before the main run continues.",
            StudyClockAlignmentWindowKind.EndBurst => "The companion is capturing a matching end-of-run clock-alignment burst before it stops the Dope runtime so clock drift over the session can be compared.",
    ...
    private void TryRecordLiveTwinState(DateTimeOffset recordedAtUtc)
    {
        ...
        {
            SetRecorderFault("Write live Dope telemetry", exception);
        }
    }
    ...
    private void UpdateParticipantSessionState()
    {
        ...
        else
        {
            ...
            ParticipantEntryDetail = "No duplicate participant folder was found under the local Dope study-data root.";
        }
        ...
        if (!string.IsNullOrWhiteSpace(_recorderFaultDetail))
        {
            ...
            RecordingSummary = "Recorder faulted during the Dope workflow.";
    ...
    private void UpdateWorkflowGuideNextActionState(int stepIndex, IReadOnlyList<WorkflowGuideActionItem> visibleActions)
    {
        ...
        (WorkflowGuideNextActionSummary, WorkflowGuideNextActionDetail) = stepIndex switch
        {
            ...
            4 => (
                "Install the pinned Dope APK if needed, then refresh.",
                "This step is only about the Dope APK identity. Bench advisories are reviewed separately afterward."),
            5 => (
                "Apply the pinned device profile, then refresh.",
                "Confirm the pinned settings and battery floors here. Remaining bench advisories stay visible but do not block Dope."),
            6 => (
            ...
            10 => (
                ...
                "Dynamic motion axis solves the axis from recorded movement. Fixed controller orientation keeps the warmed-up controller axis. Calibration remains optional on the current Dope path, but wake/proximity drift can still make the telemetry verdict look unstable."),
            11 => (
    ...
    private WorkflowGuideGateState BuildUsbDisconnectWorkflowGuideGateState()
    {
        ...
        var detail = ready
            ? _headsetStatus?.IsUsbAdbVisible == true
                ? $"Wi-Fi ADB is active on {_headsetStatus.ConnectionLabel}, and USB {_headsetStatus.VisibleUsbSerial} is still visible. Dope now treats USB visibility as advisory only, but later steps should keep reporting the Wi-Fi endpoint as the active transport."
    ...
    private WorkflowGuideGateState BuildInstalledApkWorkflowGuideGateState()
    {
        ...
        var detail = ready
            ? InstalledApkDetail
            : $"{InstalledApkDetail} If the study build is missing or wrong, install the bundled Dope Experiment APK before continuing.";
        return new WorkflowGuideGateState(level, ready ? "Correct Dope APK installed." : "Correct Dope APK not confirmed yet.", detail, ready);
    }
    ...
    private WorkflowGuideGateState BuildKioskWorkflowGuideGateState()
    {
        ...
        var detail = ready
            ? $"Dope Experiment is active in the foreground. The launch path should already have armed the keep-awake proximity override. {KioskMenuButtonAdvisory} If the controller was asleep at launch, wake it and relaunch before moving on."
        ...
        return new WorkflowGuideGateState(level, ready ? "Dope runtime is active in kiosk." : "Dope runtime is not active in kiosk yet.", detail, ready);
    }
    ...
    private (OperationOutcomeKind Level, string Summary, string Detail, bool InletReady, bool ReturnPathReady) BuildLslConnectionProbeState()
    {
        ...
        if (_headsetStatus?.IsConnected != true)
        {
            return (
                ...
                "Probe Connection needs a live headset selector before it can inspect Dope runtime routing. Reconnect USB or Wi-Fi ADB, then run the probe again.",
                false,
                false);
        }
        ...
        if (pinnedBuildBlocksProbe)
        {
            return (
                OperationOutcomeKind.Failure,
                "Pinned Dope APK is not installed or does not match the study shell baseline.",
                detail + " Install the pinned Dope APK before debugging LSL.",
                inletReady,
                false);
        }
        ...
        if (inletReady && returnPathReady)
        {
            var successSummary = !pinnedBuildReady
                ? "Quest path is live, but the pinned Dope APK is not fully verified yet."
                : deviceProfileReady
                    ? "Quest inlet is connected and the Windows return path is live."
                    : "Quest path is live, but the required Dope device profile still needs attention.";
            var successDetail = detail;
            if (!pinnedBuildReady)
            {
                successDetail += " Refresh or reinstall the pinned Dope APK before participant validation.";
            }

            if (!deviceProfileReady)
            {
                successDetail += " Apply the Dope study device profile before participant validation.";
            }
        ...
        if (inletReady)
        {
            var summary = _headsetStatus?.IsAwake == false
                ...
                : !IsStudyRuntimeForeground()
                    ? "Quest inlet is connected, but Dope is not foregrounded and the Windows return path is stale."
        ...
        if (returnPathReady)
        {
            return (
                OperationOutcomeKind.Warning,
                "Windows is receiving quest_twin_state, but Dope has not confirmed an LSL inlet yet.",
                detail + testSenderHint,
                inletReady,
                false);
        }
        ...
        return (
            ...
            !IsStudyRuntimeForeground()
                ? "Dope is not foregrounded, so neither the LSL inlet nor the Windows return path is confirmed."
                : !twinStatePublisherInventory.AnyPublisherVisible
                    ? "Dope is in front, but the Quest twin-state publisher is absent and the inlet is not confirmed."
                    : "Dope is in front, but neither the LSL inlet nor the Windows return path is confirmed.",
            detail + testSenderHint,
            inletReady,
            false);
    }
    ...
    private WorkflowGuideHazardState BuildTransportHazardState(bool inletReady, bool returnPathReady)
    {
        ...
        if (_headsetStatus.IsInWakeLimbo)
        {
            AddHazard(
                ...
                "Quest reports a Guardian, tracking-loss, or ClearActivity blocker in front of Dope. Clear it or confirm the visible scene with a fresh screenshot before trusting the live return path.");
        }
        ...
        if (!IsStudyRuntimeForeground())
        {
            AddHazard(
                OperationOutcomeKind.Failure,
                "Dope is backgrounded by the Meta shell or another app.",
                $"ADB snapshot shows {FormatOptionalValue(_headsetStatus.ForegroundPackageId, "an unknown foreground app")} instead of {_study.App.PackageId}. When Dope is not in front it can stop publishing quest_twin_state, so return-path failures in this state are expected until the app is relaunched.");
        }
        ...
        if (IsStudyRuntimeForeground() && IsProximityKeepAwakeActive() != true)
        {
            AddHazard(
                ...
                "The live Dope flow now expects the prox_close keep-awake override through launch and bench validation. On this Quest build the active override reads as virtual proximity state CLOSE, while restored normal wear-sensor behavior reads as DISABLED after automation_disable. Without the override the headset can drift into sleep or stale-twin-state conditions between guide steps even while Dope stays foregrounded.");
        }
        ...
        if (IsStudyRuntimeForeground() && inletReady && !returnPathReady)
        {
            AddHazard(
                ...
                !twinStatePublisherInventory.ExpectedPublisherVisible
                    ? "Dope still reports the expected HRV inlet as connected, but Windows cannot see a fresh quest_twin_state publisher from the Dope source_id. This is a separate failure mode from a missing inlet or a backgrounded APK."
                    : "The Quest-side inlet is still connected, but the return-path frame age exceeded 5 seconds. Keep the headset on-face, avoid USB reconnects, and relaunch Dope if fresh frames do not resume.");
        }
        else if (IsStudyRuntimeForeground() && !inletReady && returnPathReady)
        {
            AddHazard(
                OperationOutcomeKind.Warning,
                "Quest twin-state is live, but Dope has not connected the expected inlet.",
    ...
    private WorkflowGuideGateState BuildCalibrationWorkflowGuideGateState()
    {
        ...
        var detail = ready
            ...
            : CanStartBreathingCalibration
                ? "Calibration is still unstable on the current Dope APK. You can try it here, but the guide does not require it yet. Continue once the earlier LSL and particle checks are complete."
    ...
    private IReadOnlyList<WorkflowGuideCheckItem> BuildWorkflowGuideCheckItems(int stepIndex)
    {
        ...
        return stepIndex switch
        {
            ...
            [
                ...
                new WorkflowGuideCheckItem("USB visibility", _headsetStatus?.IsUsbAdbVisible == true ? $"USB {_headsetStatus.VisibleUsbSerial} is still visible." : "No USB ADB device is visible.", "USB visibility stays tracked here for awareness, but Dope now only requires the active transport to remain on the Wi-Fi endpoint.", _headsetStatus?.IsUsbAdbVisible == true ? OperationOutcomeKind.Warning : OperationOutcomeKind.Success)
            ],
            4 =>
            [
                new WorkflowGuideCheckItem("Installed Dope build", InstalledApkSummary, InstalledApkDetail, InstalledApkLevel),
                new WorkflowGuideCheckItem("Bundled Dope build", LocalApkSummary, LocalApkDetail, LocalApkLevel)
            ],
            5 =>
            [
            ...
            [
                ...
                new WorkflowGuideCheckItem("Foreground runtime", IsStudyRuntimeForeground() ? "Dope Experiment is in the foreground." : "Dope Experiment is not in the foreground yet.", HeadsetForegroundLabel, IsStudyRuntimeForeground() ? OperationOutcomeKind.Success : OperationOutcomeKind.Warning),
            ...
            [
                ...
                new WorkflowGuideCheckItem(
                    ...
                    ControllerCalibrationQualityVisible
                        ...
                        : ControllerSummary,
                    ControllerCalibrationQualityVisible
                        ? $"Calibration is optional on the current Dope APK. {ControllerCalibrationQualityExpectation} {ControllerCalibrationQualityMetrics}{(string.IsNullOrWhiteSpace(ControllerCalibrationQualityCause) ? string.Empty : $" {ControllerCalibrationQualityCause}.")} {ControllerCalibrationQualityDetail}"
                        : $"Calibration is optional on the current Dope APK. {ControllerCalibrationLabel}. {ControllerDetail}",
    ...
    private IReadOnlyList<WorkflowGuideActionItem> BuildWorkflowGuideActionItems(int stepIndex)
        => stepIndex switch
        {
            ...
            4 =>
            [
                BuildWorkflowGuideActionItem(StudyApkInstallButtonLabel, InstallStudyAppCommand, HasValidPinnedLocalApk, "Installing Dope APK..."),
                BuildWorkflowGuideActionItem("Refresh Snapshot", RefreshDeviceSnapshotCommand, true, "Refreshing Snapshot...")
            ],
            5 =>
            [
    ...
    private WorkflowGuideHazardState EvaluateKeepAwakeProximityGuideState()
    {
        ...
        return new WorkflowGuideHazardState(
            OperationOutcomeKind.Warning,
            "Keep-awake proximity override is not active.",
            "Disable proximity before launching Dope and keep it active through the remaining guide steps. On this build, normal restored wear-sensor behavior reads as virtual proximity state DISABLED after automation_disable; the active override reads as CLOSE after prox_close. Without the override the headset can slip into sleep or stale-twin-state conditions even while Dope stays foregrounded.");
    }
    ...
    {
        var minimum = GetDeviceProfileIntegerValue("dope.minimum_headset_battery_percent");
    ...
    {
        var minimum = GetDeviceProfileIntegerValue("dope.minimum_right_controller_battery_percent");
    ...
    private WorkflowGuideGateState EvaluateSoftwareIdentityGateState()
    {
        if (!SurfaceVerifiedBaselineInShell)
        {
            ...
            var advisoryDetail = string.IsNullOrWhiteSpace(PinnedBuildDetail)
                ? "Verified OS/build baseline tracking remains in code, but it is intentionally hidden in the operator shell for now. APK hash verification stays active in the Dope APK step."
        ...
        if (VerificationBaseline is null)
        {
            return new WorkflowGuideGateState(
                OperationOutcomeKind.Warning,
                "No verified Dope software baseline is recorded yet.",
        ...
        var summary = ready && PinnedBuildLevel != OperationOutcomeKind.Success
            ? "Pinned Dope APK is newer than the last verified run."
            : PinnedBuildSummary;
        var detail = ready && PinnedBuildLevel != OperationOutcomeKind.Success
            ? $"{PinnedBuildDetail} OS/build/profile still match the approved Dope baseline. Continue with this live session as the approval run for the new APK hash."
            : PinnedBuildDetail;
        return new WorkflowGuideGateState(
            ready ? OperationOutcomeKind.Success : OperationOutcomeKind.Warning,
            summary,
        ...
        string environmentHash)
    {
        ...
        builder.AppendLine(DopeClockAlignmentStreamContract.ProbeStreamName);
        builder.AppendLine(DopeClockAlignmentStreamContract.ProbeStreamType);
        builder.AppendLine(DopeClockAlignmentStreamContract.EchoStreamName);
        builder.AppendLine(DopeClockAlignmentStreamContract.EchoStreamType);
        builder.AppendLine(WorkflowClockAlignmentDurationSeconds.ToString(CultureInfo.InvariantCulture));
        builder.AppendLine(DopeClockAlignmentStreamContract.DefaultProbeIntervalMilliseconds.ToString(CultureInfo.InvariantCulture));
    ...
    private void UpdateWorkflowStatus()
    {
        ...
        WorkflowSetupSummary = setupReady
            ? profileReady
                ? "Headset, Dope build, and device profile are ready."
                : "Headset and Dope build are ready; device-profile warnings remain."
            : !headsetConnected
                ? "Connect the headset before starting the Dope protocol."
                : !wifiReady
                    ? "Enable or restore Wi-Fi ADB before starting the stable Dope flow."
                    : !wifiMatchReady
                        ? "Match the headset Wi-Fi to this PC before starting the stable Dope flow."
                    : PinnedBuildLevel == OperationOutcomeKind.Failure
                        ? "Pinned Dope build still needs attention."
        ...
        WorkflowSetupDetail =
            ...
            "Brightness, battery, and software-identity readouts stay visible as bench context, but the Dope setup path only blocks on APK readiness, Wi-Fi ADB, and Wi-Fi match. LSL remains the required bench check before participant handoff.";
        ...
        WorkflowKioskSummary = !setupReady
            ...
            : runtimeForeground
                ? QuestScreenshotLevel == OperationOutcomeKind.Warning
                    ? "Dope kiosk is up, but visual confirmation is still pending."
                    : "Dope kiosk is up and ready for bench verification."
        ...
        WorkflowKioskDetail =
            ...
            "The launch path now disables the proximity wear-sensor before Dope starts. Keep that keep-awake override active through the guide, then restore normal proximity when you are done with the live session.";
        ...
        WorkflowBenchSummary = !runtimeForeground
            ? "Bench verification is waiting for the Dope runtime."
        ...
        WorkflowBenchDetail =
            ...
            + "Recenter, particles, and controller calibration stay visible as bench warnings, but they no longer block the Dope flow.";
        ...
        WorkflowHandoffSummary = !runtimeForeground
            ...
            : !canResetCalibration
                ? "Calibration reset is still missing from the mirrored Dope APK."
        ...
        WorkflowHandoffDetail =
            (canResetCalibration
                ...
                : "Reset Calibration is unavailable in the current Dope shell metadata. ") +
        ...
        WorkflowParticipantStartDetail =
            ...
            (canStartExperiment
                ...
                : "Start Recording is unavailable because the Start Experiment command is missing from the current Dope shell metadata.");
        ...
        WorkflowParticipantEndDetail =
            ...
            (canEndExperiment
                ...
                : "Stop Recording is unavailable because End Experiment is missing from the current Dope shell metadata.");

        WorkflowRuntimePendingSummary = CanStartBreathingCalibration && canResetCalibration && canStartExperiment && canEndExperiment
            ? "Current Dope APK metadata exposes recenter, calibration start/reset, experiment start/end, and particle toggles."
            : CanStartBreathingCalibration
                ? "Current Dope APK metadata exposes recenter, calibration start, and particle toggles."
                : "Current Dope APK exposes recenter and particle toggles, but the newer session-control commands are not mirrored in this shell yet.";
        WorkflowRuntimePendingDetail =
            @"Scene contract: C:\Users\tillh\source\repos\AstralKarateDojo\Assets\Scenes\DopeControllerStudy.unity. " +
            "Participant number intake, duplicate-id warning, local session recording, screenshot archiving, Quest-side participant recording, and shared dataset/session hash handoff are now wired into the Dope shell contract.";

        if (!setupReady)
        {
            WorkflowCurrentStepLevel = WorkflowSetupLevel;
            WorkflowCurrentStepSummary = "1. Finish headset setup before starting the Dope protocol.";
    ...
    private void UpdateLiveRuntimeCard()
    {
        ...
        if (_reportedTwinState.Count == 0)
        {
            ...
            LiveRuntimeDetail = studyRuntimeForeground
                ? $"The headset still reports the Dope APK in front, but no fresh app-state frames are arriving yet. The Quest runtime may still be starting, paused, or off-face. {BuildTwinCommandTransportDetail()} {visualConfirmationHint}".Trim()
                : $"Launch the Dope runtime and wait for quest_twin_state to start publishing before relying on the live study monitor. {BuildTwinCommandTransportDetail()} {visualConfirmationHint}".Trim();
            return;
        }
    ...
    private void UpdateLslCard()
    {
        ...
        var routeNote = hasConnectedInput && !coherenceRouteIsExpected && !string.IsNullOrWhiteSpace(coherenceRouteLabel)
            ? $"The inlet is healthy, but the runtime coherence route is still {coherenceRouteLabel} (mode {coherenceRouteMode ?? "n/a"}), so Dope is not currently consuming this inlet for coherence."
            : hasConnectedInput
    ...
    private void UpdateControllerCard()
    {
        ...
        if (automaticTelemetry.AutomaticRoute)
        {
            ...
            if (_reportedTwinState.Count == 0)
            {
                ...
                ControllerDetail = "The Dope study can switch into standalone automatic breathing, but no live twin-state values have arrived yet.";
                return;
            }
        ...
        if (_reportedTwinState.Count == 0)
        {
            ...
            ControllerDetail = "The Dope study expects controller-based breathing, but no live twin-state values have arrived yet.";
            return;
        }
    ...
    private string BuildValidationCaptureActionSummary()
    {
        ...
        {
            return "This Dope shell is missing the Start or End Experiment command, so the guided validation capture cannot run from here.";
        }

        if (!IsStudyRuntimeForeground())
        {
            return "Bring the Dope runtime back to the foreground before starting the validation capture.";
        }
    ...
    private void UpdateCoherenceCard()
    {
        ...
        if (!routeMatchesExpected && lslConnected)
        {
            ...
            CoherenceDetail = "This explains the apparent mismatch: the headset is listening on the expected LSL inlet, but Dope is not currently using that inlet as its active coherence source. A hotload/config path is still selecting a different coherence route.";
            return;
        }
    ...
    private void UpdatePerformanceCard()
    {
        ...
        if (!fps.HasValue && !frameMs.HasValue && !targetFps.HasValue)
        {
            ...
            PerformanceDetail = "The Dope runtime is live, but it has not published fps or frame-time values yet.";
            return;
        }
    ...
    private StudyStatusRow CreateLslValueRow()
    {
        ...
        return new StudyStatusRow(
            ...
            string.Empty,
            "The Dope runtime confirms inlet connectivity here, but the current public twin-state frame only echoes the routed biofeedback value when signal mirroring is enabled.",
            OperationOutcomeKind.Preview);
    }
