...
public sealed class DopeVisualProfilesWorkspaceViewModel : ObservableObject, IDisposable
{
    private const string BundledBaselineProfileId = "__bundled_dope_visual_baseline__";
    private const string BundledProfileIdPrefix = "__bundled_dope_visual_profile__::";
    ...
    private readonly DopeVisualTuningCompiler? _compiler;
    private readonly DopeVisualProfileStore? _profileStore;
    private readonly DopeVisualProfileApplyStateStore? _applyStateStore;
    private readonly DopeVisualProfileStartupStateStore? _startupStateStore;
    ...
    private DopeVisualProfileApplyRecord? _lastApplyRecord;
    private DopeVisualProfileListItemViewModel? _selectedProfile;
    private DopeVisualProfileListItemViewModel? _compareProfile;
    ...
    private DopeVisualTuningDocument? _draftSourceDocument;
    private string _draftSourceProfileId = string.Empty;
    private string _draftSourceLabel = "Bundled Dope Baseline";
    private string _librarySummary = "Loading Dope visual profiles...";
    private string _libraryDetail = "The Dope shell stores one self-describing json file per visual profile.";
    private OperationOutcomeKind _libraryLevel = OperationOutcomeKind.Preview;
    private string _applySummary = "No Dope visual profile has been applied yet.";
    ...
    private string _lastCompiledCsvPath = "No Dope visual hotload CSV written yet.";
    private string _templatePathLabel = "Bundled Dope visual tuning template not found.";
    private string _libraryRootLabel = string.Empty;
    private DopeVisualProfileStartupState? _startupState;

    public DopeVisualProfilesWorkspaceViewModel(
        ...
        ITwinModeBridge? twinBridge = null)
    {
        ...
        try
        {
            string? templatePath = AppAssetLocator.TryResolveDopeVisualTuningTemplatePath();
            if (!string.IsNullOrWhiteSpace(templatePath))
            {
                var templateJson = File.ReadAllText(templatePath);
                _compiler = new DopeVisualTuningCompiler(templateJson);
                _profileStore = new DopeVisualProfileStore(_compiler);
                _applyStateStore = new DopeVisualProfileApplyStateStore(_study.Id);
                _startupStateStore = new DopeVisualProfileStartupStateStore(_study.Id);
                ...
                BuildEditorGroups();
                _librarySummary = "Dope visual profile library ready.";
                _libraryDetail = "The bundled Dope baseline is always available. Table edits stay in the working draft until you explicitly save them to the library or pin a saved profile for launch.";
                _libraryLevel = OperationOutcomeKind.Success;
            }
            else
            {
                _librarySummary = "Dope visual profile template missing.";
                _libraryDetail = "The shell could not resolve the bundled dope-visual-tuning-v1.template.json asset.";
                _libraryLevel = OperationOutcomeKind.Warning;
            }
        }
        catch (Exception ex)
        {
            _librarySummary = "Dope visual profile library unavailable.";
        ...
        ResetFieldCommand = new AsyncRelayCommand(ResetFieldAsync, parameter => parameter is DopeVisualProfileFieldViewModel && IsAvailable);
    }
    ...
    public ObservableCollection<DopeVisualProfileListItemViewModel> Profiles { get; } = new();
    public ObservableCollection<DopeVisualProfileGroupViewModel> Groups { get; } = new();
    public ObservableCollection<DopeVisualComparisonRowViewModel> ComparisonRows { get; } = new();
    ...
    public DopeVisualProfileListItemViewModel? SelectedProfile
    {
    ...
    public DopeVisualProfileListItemViewModel? CompareProfile
    {
        get => _compareProfile;
        set
        {
    ...
    public string DraftSourceDetail
        => _draftSourceDocument is null
            ? "Select the bundled Dope baseline or a saved profile to create a runtime working draft."
    ...
    public bool IsSelectedProfileStartupDefault
        => _compiler is not null &&
           ...
           DopeVisualStartupSnapshotResolver.MatchesCurrentSelection(
               _compiler,
               _startupState,
               SelectedProfile.Id,
    ...
    public string StartupProfileSummary
        => _startupState is null
            ? "Next-launch override: bundled Dope baseline."
            : $"Next-launch override: {_startupState.ProfileName}.";

    public string StartupProfileDetail
        => _startupState is null
            ? "Dope launches on the bundled baseline visual state unless you explicitly pin a saved profile below. Applying the runtime draft does not change future launches."
            : $"The saved launch profile '{_startupState.ProfileName}' is the device-side Dope startup baseline. Table edits and current-session applies do not change that launch pin until you save a profile and set it for launch.";
    ...
    public DopeVisualSessionSnapshot CaptureSessionSnapshot()
    {
        ...
        if (_compiler is not null && _lastApplyRecord is not null && selectedMatchesLastApplied)
        {
            ...
            var computation = DopeVisualRowConfirmationResolver.Compute(fields, _lastApplyRecord, confirmationRows);
            hasUnappliedEdits = computation.ChangedSinceApplyCount > 0;
        }

        return new DopeVisualSessionSnapshot(
            IsAvailable,
            ApplyLevel,
            ApplySummary,
            ApplyDetail,
    ...
    private void BuildEditorGroups()
    {
        ...
        {
            var group = new DopeVisualProfileGroupViewModel(groupTitle);
            foreach (var control in _compiler.TemplateDocument.Controls.Where(control => ResolveGroupTitle(control.Id) == groupTitle))
            {
                group.Fields.Add(new DopeVisualProfileFieldViewModel(control, OnFieldValueChanged));
            }
    ...
    private DopeVisualProfileListItemViewModel CreateBundledBaselineListItem()
    {
        if (_compiler is null)
        {
            throw new InvalidOperationException("Dope visual tuning compiler is not available.");
        }

        var bundledDocument = _compiler.TemplateDocument;
        var payload = _compiler.Serialize(bundledDocument);
        return new DopeVisualProfileListItemViewModel(
            new DopeVisualProfileRecord(
            ...
            displayLabelOverride: "Bundled Dope Baseline",
            modifiedLabelOverride: "Bundled with the APK");
    }

    private bool CanSwitchToProfile(DopeVisualProfileListItemViewModel? nextProfile)
    {
    ...
    private bool ComputeHasUnsavedDraftChanges()
    {
        ...
        return !DopeVisualDocumentComparer.Matches(_compiler, currentDocument!, _draftSourceDocument);
    }
    ...
    private async Task ReloadProfilesAsync(string? selectProfileId = null)
    {
        ...
        await DispatchAsync(() =>
        {
            ...
            try
            {
                ...
                {
                    Profiles.Add(new DopeVisualProfileListItemViewModel(
                        bundledProfile,
                ...
                {
                    Profiles.Add(new DopeVisualProfileListItemViewModel(profile));
                }
                ...
                if (savedProfiles.Count == 0 && bundledProfiles.Count == 0)
                {
                    ...
                    LibrarySummary = "No bundled or local Dope visual profiles yet.";
                    LibraryDetail = "The bundled Dope baseline is still available below. Use Save Draft As New Profile to create the first editable copy in the local library, then sync chosen profiles into the repo bundle when you want to ship them.";
                ...
                LibrarySummary = $"Loaded {bundledProfiles.Count.ToString(CultureInfo.InvariantCulture)} bundled Dope visual profile(s), {savedProfiles.Count.ToString(CultureInfo.InvariantCulture)} local saved profile(s), plus the bundled baseline.";
    ...
    private void LoadSelectedProfileIntoEditor(DopeVisualProfileListItemViewModel? profile)
    {
        if (profile is null)
        {
            _draftSourceDocument = null;
            _draftSourceProfileId = string.Empty;
            _draftSourceLabel = "Bundled Dope Baseline";
    ...
    private DopeVisualProfileRecord? CreateCurrentProfileRecord()
    {
    ...
    private DopeVisualProfileRecord? ResolveEffectiveProfileRecord(DopeVisualProfileRecord? currentProfile)
    {
    ...
    private bool CurrentDraftMatchesLastApplyRequest(DopeVisualTuningDocument? currentDocument = null)
    {
        ...
        return currentDocument is not null &&
               DopeVisualDocumentComparer.MatchesControlValues(
                   _compiler,
                   currentDocument,
                   _lastApplyRecord.RequestedValues);
    }

    private DopeVisualProfileSnapshot? CaptureCurrentSnapshot()
    {
        ...
        return new DopeVisualProfileSnapshot(
            SelectedProfile,
    ...
    private async Task<DopeVisualProfileRecord?> SaveSnapshotAsync(
        DopeVisualProfileSnapshot snapshot,
        string? existingPath,
        string summary)
    {
        if (_profileStore is null)
        ...
        catch (Exception ex)
        {
            await DispatchAsync(() =>
            {
                LibrarySummary = "Saving Dope visual profile failed.";
    ...
    private async Task NewFromBaselineAsync()
    {
        ...
        var created = await _profileStore.CreateFromTemplateAsync(BuildUniqueProfileName("Dope Visual Profile"));
    ...
    private async Task SaveSelectedAsync()
    {
        ...
        var saved = await SaveSnapshotAsync(
            ...
            "Saved changes to the selected Dope visual profile.");
        if (saved is null)
        {
            return;
        }
        ...
        {
            _startupState = new DopeVisualProfileStartupState(
                saved.Id,
        ...
        {
            _lastApplyRecord = new DopeVisualProfileApplyRecord(
                saved.Id,
                saved.Document.Profile.Name,
                saved.FileHash,
        ...
        await ReloadProfilesAsync(saved.Id);
        LibrarySummary = "Saved changes to the selected Dope visual profile.";
        LibraryDetail = saved.FilePath;
        LibraryLevel = OperationOutcomeKind.Success;
    }
    ...
    private async Task ImportProfileAsync()
    {
        ...
        {
            Title = "Import Dope Visual Profile",
        ...
        {
            LibrarySummary = "Dope visual profile import failed.";
            LibraryDetail = ex.Message;
            LibraryLevel = OperationOutcomeKind.Failure;
        }
    }
    ...
    private async Task ExportSelectedAsync()
    {
        ...
        {
            Title = "Export Dope Visual Profile",
            Filter = "JSON files (*.json)|*.json|All files (*.*)|*.*",
        ...
        LibrarySummary = "Dope visual profile exported.";
    ...
    private async Task SetStartupProfileAsync()
    {
        ...
        var saved = SelectedProfile.ToRecord();
        var startupState = new DopeVisualProfileStartupState(
            saved.Id,
            saved.Document.Profile.Name,
            DateTimeOffset.UtcNow,
            saved.Document.Profile.Notes,
    ...
    internal DopeVisualStartupHotloadPlan? CapturePinnedStartupHotloadPlan()
    {
        if (_compiler is null)
        {
            return null;
        ...
        return new DopeVisualStartupHotloadPlan(
            startupRecord,
            compiled.Entries,
            new Dictionary<string, double?>(previousReportedValues, StringComparer.OrdinalIgnoreCase));
    }

    internal void TrackPinnedStartupLaunch(DopeVisualStartupHotloadPlan plan, DateTimeOffset appliedAtUtc, string? csvPath = null)
    {
    ...
    {
        DopeVisualProfileRecord? startupRecord = null;
        await DispatchAsync(() =>
        {
        ...
        if (startupRecord is null)
        {
            await DispatchAsync(() =>
            {
                ApplySummary = "APK launch is using the bundled Dope baseline visual state.";
                ApplyDetail = "No saved startup profile is pinned, so Dope keeps the bundled baseline until you apply another profile.";
    ...
    private async Task DeleteSelectedAsync()
    {
        ...
        if (MessageBox.Show(
                $"Delete '{SelectedProfile.Document.Profile.Name}' from the Dope visual profile library?",
                "DOPE Companion",
                MessageBoxButton.YesNo,
                MessageBoxImage.Warning) != MessageBoxResult.Yes)
        {
    ...
    private async Task ApplySelectedAsync()
    {
        ...
        var draftRecord = new DopeVisualProfileRecord(
            SelectedProfile.Id,
        ...
        bool preferTwinRuntimePublish = false)
    {
        ...
        try
        {
            ...
            var runtimeProfile = new RuntimeConfigProfile(
                ...
                $"Dope Visual Profile - {saved.Document.Profile.Name}",
                string.Empty,
                DateTimeOffset.UtcNow.ToString("yyyy.MM.dd.HHmmss", CultureInfo.InvariantCulture),
                "study",
                false,
                $"Compiled from {saved.Document.Profile.Name}. Only the Dope-approved visual envelope fields plus the simplified tracer wrapper were changed.",
    ...
    {
        if (parameter is DopeVisualProfileFieldViewModel field)
        {
    ...
    private void RefreshComparisonState(bool syncCurrentValueText = false)
    {
        ...
        try
        {
            ...
            var rows = DopeVisualComparisonRowBuilder.Build(_compiler, fields, startupComparisonDocument);
            ...
            var rowConfirmationStates = new Dictionary<string, DopeVisualRowConfirmationState>(StringComparer.OrdinalIgnoreCase);
            ...
            if (selectedMatchesLastApplyProfile)
            {
                ...
                var computation = DopeVisualRowConfirmationResolver.Compute(fieldsById.Values, _lastApplyRecord!, confirmationRows);
                rowConfirmationStates = new Dictionary<string, DopeVisualRowConfirmationState>(computation.States, StringComparer.OrdinalIgnoreCase);
                ...
                else if (changedSinceApplyCount > 0)
                {
                    ApplySummary = changedSinceApplyCount == 1
                        ? "1 Dope parameter value changed since the last apply."
                        : $"{changedSinceApplyCount.ToString(CultureInfo.InvariantCulture)} Dope parameter values changed since the last apply.";
                ...
                else
                {
                    ...
                    else
                    {
                        ApplyDetail = confirmation.WaitingCount > 0
                            ? $"Applied {_lastApplyRecord!.ProfileName} at {_lastApplyRecord.AppliedAtUtc.ToLocalTime():HH:mm:ss}. Upload succeeded; waiting for a fresh quest_twin_state report to mirror the requested Dope visual and tracer values."
            ...
            {
                ApplySummary = "No Dope visual profile has been applied yet.";
                ...
                ComparisonRows.Select(row => row.Field.Id).SequenceEqual(rows.Select(row => row.Id), StringComparer.OrdinalIgnoreCase) is false)
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    ComparisonRows.Add(new DopeVisualComparisonRowViewModel(
        ...
        IDictionary<string, DopeVisualRowConfirmationState> rowConfirmationStates)
    {
        ...
        {
            if (!IsTracerControlId(pair.Key) || pair.Value.State != DopeVisualConfirmationState.Waiting)
            {
                continue;
            }

            rowConfirmationStates[pair.Key] = DopeVisualRowConfirmationResolver.MissingTelemetryConfirmationState;
            missingCount++;
        }
    ...
    private bool ShouldFlagMissingTracerTelemetry(IReadOnlyDictionary<string, DopeVisualConfirmationRow> confirmationRows)
    {
        ...
        {
            if (!IsTracerControlId(row.Id) || row.State != DopeVisualConfirmationState.Waiting)
            {
                continue;
            }
    ...
    {
        var seed = string.IsNullOrWhiteSpace(baseName) ? "Dope Visual Profile" : baseName.Trim();
    ...
    private async Task<IReadOnlyList<DopeVisualProfileRecord>> LoadBundledProfilesAsync(CancellationToken cancellationToken = default)
    {
        if (_compiler is null)
        {
        ...
        var bundledRoot = AppAssetLocator.TryResolveBundledDopeVisualProfilesRoot();
    ...
    private DopeVisualProfileListItemViewModel? ResolvePinnedStartupProfile()
        => _startupState is null
            ? null
            : Profiles.FirstOrDefault(profile =>
                string.Equals(profile.Id, _startupState.ProfileId, StringComparison.OrdinalIgnoreCase));

    private DopeVisualTuningDocument ResolveStartupComparisonDocument()
    {
        if (_compiler is null)
        {
            throw new InvalidOperationException("Dope visual tuning compiler is not available.");
        ...
        return DopeVisualStartupSnapshotResolver.ResolveDocument(
            _compiler,
            _startupState,
            ResolvePinnedStartupProfile()?.Document);
    }

    private DopeVisualProfileRecord? CreateStartupProfileRecord()
    {
        ...
        DopeVisualTuningDocument startupDocument;
        try
        {
        ...
        out string? error)
    {
        ...
        return DopeVisualCurrentDocumentResolver.TryCreate(
            _compiler,
            SelectedProfileName,
        ...
        else if (!headsetStatus.IsTargetForeground)
        {
            detailParts.Add(
                headsetStatus.IsTargetRunning
                    ? "The Dope APK is running but not currently foreground, so the visual change may not be immediately visible."
                    : "The Dope APK is not currently running in foreground. The staged file should load the next time the runtime starts.");
        }

        if (outcome.Kind != OperationOutcomeKind.Failure)
        {
            detailParts.Add("Twin-state confirmation will track the approved Dope visual and tracer values after the headset reports them.");
        }
        ...
        string csvPath)
    {
        ...
        detailParts.Add($"Compiled CSV: {csvPath}.");
        detailParts.Add(
            "The Dope runtime must be awake and visible enough to poll runtime_hotload/runtime_overrides.csv before a visual apply can take effect. Wake the headset or use the bench-tools proximity hold, then apply again.");
        return string.Join(" ", detailParts);
    }
