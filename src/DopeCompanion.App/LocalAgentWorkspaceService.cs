...
internal sealed class LocalAgentWorkspaceService
{
    ...
    private LocalAgentWorkspaceSnapshot BuildSnapshot()
    {
        ...
        return new LocalAgentWorkspaceSnapshot(
            ...
            PowerShellCliWrapperPath: Path.Combine(_workspaceRoot, "dope-companion.ps1"),
            CmdCliWrapperPath: Path.Combine(_workspaceRoot, "dope-companion.cmd"),
    ...
    private string BuildReadme(LocalAgentWorkspaceSnapshot snapshot)
    {
        ...
        builder.AppendLine();
        builder.AppendLine("Open your local agent in this folder if you installed DOPE Companion from the guided installer and do not have a repo checkout.");
        builder.AppendLine("This workspace mirrors the bundled CLI, the CLI reference, and the Dope example catalogs into the same host-visible operator-data root the installed app uses, so the agent does not need access to the protected WindowsApps install.");
        builder.AppendLine();
        ...
        AppendRelativeFileIfPresent(builder, snapshot.RootPath, Path.Combine(snapshot.StudyShellRootPath, "dope-projected-feed-colorama.json"));
        ...
        if (snapshot.HasBundledCli)
        {
            ...
            builder.AppendLine("- Use `.\\dope-companion.ps1` from PowerShell or `dope-companion.cmd` from `cmd.exe`; both wrappers preload the mirrored sample-root overrides and the bundled liblsl path before invoking the bundled CLI.");
        }
        else
        {
    ...
    private string BuildLocalPaths(LocalAgentWorkspaceSnapshot snapshot)
    {
        ...
        builder.AppendLine($"- Dope visual profiles: `{CompanionOperatorDataLayout.DopeVisualProfilesRootPath}`");
        builder.AppendLine($"- Dope controller profiles: `{CompanionOperatorDataLayout.DopeControllerBreathingProfilesRootPath}`");
    ...
    {
        var builder = new StringBuilder();
        builder.AppendLine($"$env:DOPE_QUEST_SESSION_KIT_ROOT = \"{EscapeForPowerShell(snapshot.QuestSessionKitRootPath)}\"");
        builder.AppendLine($"$env:DOPE_STUDY_SHELL_ROOT = \"{EscapeForPowerShell(snapshot.StudyShellRootPath)}\"");
        builder.AppendLine($"$env:DOPE_OSCILLATOR_CONFIG_ROOT = \"{EscapeForPowerShell(snapshot.OscillatorConfigRootPath)}\"");
        ...
        builder.AppendLine("$bundledCliExe = Join-Path $bundledCliRoot 'dope-companion.exe'");
        builder.AppendLine("if (Test-Path $bundledLslDll) {");
        builder.AppendLine("    $env:DOPE_LSL_DLL = $bundledLslDll");
        builder.AppendLine("} elseif (Test-Path $bundledLslRuntimeDll) {");
        builder.AppendLine("    $env:DOPE_LSL_DLL = $bundledLslRuntimeDll");
        builder.AppendLine("}");
        builder.AppendLine("if (Test-Path $bundledCliExe) {");
        builder.AppendLine("    $env:PATH = \"$bundledCliRoot;$env:PATH\"");
        builder.AppendLine("}");
        builder.AppendLine("Write-Host \"Set DOPE companion CLI sample-root overrides and bundled liblsl path for this PowerShell session.\"");
        return builder.ToString();
    }
    ...
    private static string BuildCmdEnvScript(LocalAgentWorkspaceSnapshot snapshot)
    {
        ...
        builder.AppendLine($"set \"DOPE_QUEST_SESSION_KIT_ROOT={snapshot.QuestSessionKitRootPath}\"");
        builder.AppendLine($"set \"DOPE_STUDY_SHELL_ROOT={snapshot.StudyShellRootPath}\"");
        builder.AppendLine($"set \"DOPE_OSCILLATOR_CONFIG_ROOT={snapshot.OscillatorConfigRootPath}\"");
        builder.AppendLine($"set \"{CompanionOperatorDataLayout.RootOverrideEnvironmentVariable}={CompanionOperatorDataLayout.RootPath}\"");
        builder.AppendLine($"if exist \"{LslRuntimeLayout.GetLocalDllPath(snapshot.BundledCliRootPath)}\" set \"DOPE_LSL_DLL={LslRuntimeLayout.GetLocalDllPath(snapshot.BundledCliRootPath)}\"");
        builder.AppendLine($"if not defined DOPE_LSL_DLL if exist \"{LslRuntimeLayout.GetRuntimeDllPath(snapshot.BundledCliRootPath)}\" set \"DOPE_LSL_DLL={LslRuntimeLayout.GetRuntimeDllPath(snapshot.BundledCliRootPath)}\"");
        builder.AppendLine($"set \"PATH={snapshot.BundledCliRootPath};%PATH%\"");
        builder.AppendLine("echo Set DOPE companion CLI sample-root overrides and bundled liblsl path for this cmd.exe session.");
        return builder.ToString();
    }
    ...
    private static string BuildPowerShellCliWrapper(LocalAgentWorkspaceSnapshot snapshot)
    {
        ...
        builder.AppendLine("$bundledLslRuntimeDll = Join-Path $bundledCliRoot 'runtimes\\win-x64\\native\\lsl.dll'");
        builder.AppendLine("$bundledCliExe = Join-Path $bundledCliRoot 'dope-companion.exe'");
        builder.AppendLine("$bundledCliDll = Join-Path $bundledCliRoot 'dope-companion.dll'");
        builder.AppendLine("if (Test-Path $bundledLslDll) {");
        builder.AppendLine("    $env:DOPE_LSL_DLL = $bundledLslDll");
        builder.AppendLine("} elseif (Test-Path $bundledLslRuntimeDll) {");
        builder.AppendLine("    $env:DOPE_LSL_DLL = $bundledLslRuntimeDll");
        ...
        builder.AppendLine("}");
        builder.AppendLine();
        builder.AppendLine("Write-Error \"Bundled DOPE companion CLI not found under $bundledCliRoot. Open LOCAL_PATHS.md in this workspace for alternatives.\"");
    ...
    private static string BuildCmdCliWrapper(LocalAgentWorkspaceSnapshot snapshot)
    {
        ...
        builder.AppendLine("if exist \"%BUNDLED_CLI_ROOT%\\lsl.dll\" set \"DOPE_LSL_DLL=%BUNDLED_CLI_ROOT%\\lsl.dll\"");
        builder.AppendLine("if not defined DOPE_LSL_DLL if exist \"%BUNDLED_CLI_ROOT%\\runtimes\\win-x64\\native\\lsl.dll\" set \"DOPE_LSL_DLL=%BUNDLED_CLI_ROOT%\\runtimes\\win-x64\\native\\lsl.dll\"");
        builder.AppendLine("set \"PATH=%BUNDLED_CLI_ROOT%;%PATH%\"");
        builder.AppendLine("if exist \"%BUNDLED_CLI_ROOT%\\dope-companion.exe\" (");
        builder.AppendLine("  \"%BUNDLED_CLI_ROOT%\\dope-companion.exe\" %*");
        builder.AppendLine("  exit /b %ERRORLEVEL%");
        builder.AppendLine(")");
        builder.AppendLine("if exist \"%BUNDLED_CLI_ROOT%\\dope-companion.dll\" (");
        builder.AppendLine("  dotnet \"%BUNDLED_CLI_ROOT%\\dope-companion.dll\" %*");
        builder.AppendLine("  exit /b %ERRORLEVEL%");
        builder.AppendLine(")");
        builder.AppendLine("echo Bundled DOPE companion CLI not found under \"%BUNDLED_CLI_ROOT%\". Open LOCAL_PATHS.md in this workspace for alternatives.");
    ...
    {
        var builder = new StringBuilder();
        builder.AppendLine("You are looking at a local agent workspace exported from an installed copy of DOPE Companion.");
        ...
        builder.AppendLine("- samples/study-shells/dope-projected-feed-colorama.json");
        ...
        builder.AppendLine("- `.\\dope-companion.ps1 --help`");
        builder.AppendLine("- `.\\dope-companion.ps1 windows-env analyze`");
        ...
        builder.AppendLine("2. Which Dope tasks are CLI-first versus GUI-first.");
    ...
    {
        var bundledCliExe = Path.Combine(bundledCliRoot, "dope-companion.exe");
        if (File.Exists(bundledCliExe))
        {
            return bundledCliExe;
        }

        var bundledCliDll = Path.Combine(bundledCliRoot, "dope-companion.dll");
