...
namespace DopeCompanion.PreviewInstaller;
...
internal sealed class InstallerStatusForm : Form
{
        ...
        string releasePageUri)
    {
        ...
        _titleLabel = new Label
        {
            ...
            MaximumSize = new Size(800, 0),
            Text = "Install Or Update DOPE Companion",
    ...
    private async Task RunInstallAsync()
    {
        ...
        try
        {
            ...
            _footerLabel.Text = string.IsNullOrWhiteSpace(result.ToolingWarning)
                ? "The packaged install is ready. The helper attempted to open DOPE Companion automatically. If Windows does not bring it forward, launch it from the Start menu and continue with the standard DOPE operator workflow."
                : $"{result.ToolingWarning} You can rerun the helper later or use `dope-companion tooling install-official` after the app is installed.";
    ...
    private static Image? LoadEmbeddedLogo()
    {
        ...
        using var stream = assembly.GetManifestResourceStream("DopeCompanion.PreviewInstaller.Assets.dope-companion.png");
