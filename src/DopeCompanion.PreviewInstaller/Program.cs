...
internal static class Program
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        progress.Report(new InstallerProgressUpdate(
            ...
            "Checking whether a previous packaged DOPE Companion install is already registered on this machine.",
            87));
    ...
    private static string BuildCompletionSummary(PreviewPackageInstallResult installResult)
    {
        ...
        {
            return $"DOPE Companion {installResult.InstalledVersion} replaced the previous install.";
        }

        if (installResult.RemovedLegacyInstall)
        {
            return $"DOPE Companion {installResult.InstalledVersion} installed and retired the legacy packaged entry.";
        }
        ...
        if (installResult.UpdatedExistingInstall)
        {
            return string.Equals(installResult.PreviousVersion, installResult.InstalledVersion, StringComparison.OrdinalIgnoreCase)
                ? $"DOPE Companion {installResult.InstalledVersion} is installed."
                : $"DOPE Companion updated to {installResult.InstalledVersion}.";
        }

        return $"DOPE Companion {installResult.InstalledVersion} installed.";
    }
